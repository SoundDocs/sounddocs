...
const tetrisShapes = {
  I: { color: '#2dd4bf', blocks: [[[[0,0],[1,0],[2,0],[3,0]]],[[[0,0],[0,1],[0,2],[0,3]]]] },
  O: { color: '#facc15', blocks: [[[[0,0],[0,1],[1,0],[1,1]]]] },
  T: { color: '#a78bfa', blocks: [[[[0,1],[1,0],[1,1],[2,1]]],[[[1,0],[0,1],[1,1],[1,2]]],[[[0,0],[1,0],[2,0],[1,1]]],[[[0,1],[1,1],[1,2],[1,0]]]] },
  L: { color: '#fb923c', blocks: [[[[0,0],[0,1],[0,2],[1,2]]],[[[0,0],[1,0],[2,0],[0,1]]],[[[0,0],[1,0],[1,1],[1,2]]],[[[2,0],[0,1],[1,1],[2,1]]]] },
  J: { color: '#60a5fa', blocks: [[[[1,0],[1,1],[1,2],[0,2]]],[[[0,0],[0,1],[1,1],[2,1]]],[[[0,0],[0,1],[0,2],[1,0]]],[[[0,0],[1,0],[2,0],[2,1]]]] },
  S: { color: '#4ade80', blocks: [[[[1,0],[2,0],[0,1],[1,1]]],[[[0,0],[1,0],[1,1],[2,1]]]] },
  Z: { color: '#f87171', blocks: [[[[0,0],[1,0],[1,1],[2,1]]],[[[1,0],[0,1],[1,1],[0,2]]]] },
};
const shapeKeys = Object.keys(tetrisShapes);

function generateTetrisGrid(mapWidth: number, mapHeight: number): string[][] {
    const grid: (string | null)[][] = Array(mapHeight).fill(null).map(() => Array(mapWidth).fill(null));
    ...
    for (let r = 0; r < mapHeight; r++) {
        for (let c = 0; c < mapWidth; c++) {
            if (grid[r][c] === null) {
                const shapeKey = shapeKeys[Math.floor(random() * shapeKeys.length)] as keyof typeof tetrisShapes;
...
export function createLedPixelMapSvg(props: LedMapSvgProps): string {
  const {
    width, height, projectName, screenName, mapWidth, mapHeight,
    aspectWidth, aspectHeight, displayMode, showScreenInfo,
    showStats, showFooter, showGuides
  } = props;
  ...
  const panelsSvg = panels.map((panel, i) => `
    <g key="${i}">
      <rect
        x="${panel.x}" y="${panel.y}"
