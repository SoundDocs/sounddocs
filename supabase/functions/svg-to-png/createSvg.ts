export function createPixelMapSvg({
  ...
  const totalPatchesWidth =
    colorPatches.length * patchSize + (colorPatches.length - 1) * patchSpacing;
  ...
  const patchesSvg = colorPatches
    .map((patch, index) => {
      const patchX = index * (patchSize + patchSpacing);
      return `
      <g>
        <rect x="${patchX}" y="0" width="${patchSize}" height="${patchSize}" fill="${patch.color}" stroke="#A3A3A3" stroke-width="2" rx="4" />
        <text x="${patchX + patchSize / 2}" y="${patchSize + labelFontSize + 10}" font-family="Inter" font-size="${labelFontSize}" fill="#A3A3A3" text-anchor="middle">
  ...
  return `
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- Guides and Markers -->
      <line x1="${width / 2}" y1="0" x2="${width / 2}" y2="${height}" stroke="#A3A3A3" stroke-width="2" stroke-dasharray="10 10" />
      <line x1="0" y1="${height / 2}" x2="${width}" y2="${height / 2}" stroke="#A3A3A3" stroke-width="2" stroke-dasharray="10 10" />
      <line x1="0" y1="0" x2="${width}" y2="${height}" stroke="#FFFFFF" stroke-width="2" stroke-dasharray="10 10" />
      <line x1="${width}" y1="0" x2="0" y2="${height}" stroke="#FFFFFF" stroke-width="2" stroke-dasharray="10 10" />
      ...
      <!-- Screen Name -->
      <text x="${width / 2}" y="${height / 2 - 40 * scale}" font-family="Inter" font-size="${titleFontSize}" fill="#FFFFFF" text-anchor="middle">
        ${screen_name.toUpperCase()}
      </text>

      <!-- Logo -->
      <g transform="translate(${width / 2}, ${height / 2}) scale(${logoScale})">
        <path d="m19 21-7-4-7 4V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z" transform="translate(-12, -12)" stroke="#9E7FFF" stroke-width="${logoStrokeWidth}" fill="none" stroke-linecap="round" stroke-linejoin="round" />
      </g>

      <!-- Resolution -->
      <text x="${width / 2}" y="${height / 2 + 55 * scale}" font-family="Inter" font-size="${resolutionFontSize}" fill="#A3A3A3" text-anchor="middle">
        W ${width} x H ${height}
      </text>

      <!-- Footer Branding -->
      <rect y="${height - 40}" width="${width}" height="40" fill="rgba(23, 23, 23, 0.85)" />
      <text x="15" y="${height - 15}" font-family="Inter" font-size="16" fill="#A3A3A3" text-anchor="start">
        ${project_name}
      </text>
      <text x="${width - 15}" y="${height - 15}" font-family="Inter" font-size="16" fill="#A3A3A3" text-anchor="end">
