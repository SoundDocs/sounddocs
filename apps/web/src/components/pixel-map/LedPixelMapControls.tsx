...
const LedPixelMapControls: React.FC<LedPixelMapControlsProps> = ({
  mapData,
  setMapData,
  previewOptions,
  setPreviewOptions,
  ...
  const handlePreviewOptionsChange = (
  ...
  const ToggleSwitch: React.FC<{ name: keyof PreviewOptions; label: string }> = ({
    name,
    label,
  }) => (
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4 pt-4 border-t border-border">
          ...
          <div>
            ...
            <select
              name="displayMode"
              id="displayMode"
              value={previewOptions.displayMode}
              onChange={handlePreviewOptionsChange}
              className={inputClasses}
            >
        ...
        <div className="text-sm text-textSecondary space-y-2 pt-4 border-t border-border">
          <div className="flex justify-between">
            <span>Total Panels:</span> <span className="font-mono text-text">{totalPanels}</span>
          </div>
          <div className="flex justify-between">
            <span>Screen Dimensions:</span>{" "}
            <span className="font-mono text-text">
              {screenWidth > 0 && screenHeight > 0 ? `${screenWidth}x${screenHeight}px` : "N/A"}
            </span>
          </div>
          <div className="flex justify-between">
            <span>Aspect Ratio:</span>{" "}
            <span className="font-mono text-text">
              {screenWidth > 0 && screenHeight > 0 ? `${aspectWidth}:${aspectHeight}` : "N/A"}
            </span>
          </div>
          <div className="flex justify-between">
            <span>Total Pixels:</span>{" "}
            <span className="font-mono text-text">{totalPixels.toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
