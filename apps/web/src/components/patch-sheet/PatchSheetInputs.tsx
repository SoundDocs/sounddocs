...
const PatchSheetInputs: React.FC<PatchSheetInputsProps> = ({ inputs, updateInputs }) => {
  ...
  useEffect(() => {
    if (user) {
      fetchUserCustomSuggestions(user.id, "input_")
        .then((suggestionsMap) => {
          setUserCustomDevices(suggestionsMap[FIELD_TYPES.INPUT_DEVICE] || []);
          setUserCustomAnalogSnakeTypes(
            suggestionsMap[FIELD_TYPES.INPUT_CONN_ANALOG_SNAKE_TYPE] || [],
          );
          setUserCustomDigitalSnakeTypes(
            suggestionsMap[FIELD_TYPES.INPUT_CONN_DIGITAL_SNAKE_TYPE] || [],
          );
  ...
  const updateParentInputs = () => {
    const updatedInputs = inputs.map((input) =>
      editModeInputs[input.id] ? { ...editingInputs[input.id] } : input,
    );
  ...
  const handleDeleteInput = (id: string) => {
    ...
    if (inputToDelete?.isStereo && inputToDelete?.stereoChannelNumber) {
      updatedInputs = updatedInputs.map((input) =>
        input.channelNumber === inputToDelete.stereoChannelNumber
          ? { ...input, isStereo: false, stereoChannelNumber: undefined }
          : input,
      );
    }
    updatedInputs = updatedInputs.map((input) =>
      input.stereoChannelNumber === inputToDelete?.channelNumber
        ? { ...input, isStereo: false, stereoChannelNumber: undefined }
        : input,
    );
    updateInputs(updatedInputs);
  ...
  const handleSaveInput = (id: string) => {
    ...
    if (updatedInput.isStereo && updatedInput.stereoChannelNumber) {
      currentInputs = currentInputs.map((input) =>
        input.channelNumber === updatedInput.stereoChannelNumber && input.id !== id
          ? { ...input, isStereo: true, stereoChannelNumber: updatedInput.channelNumber }
          : input,
      );
    } else if (!updatedInput.isStereo) {
      currentInputs = currentInputs.map((input) =>
        input.stereoChannelNumber === updatedInput.channelNumber
          ? { ...input, isStereo: false, stereoChannelNumber: undefined }
          : input,
      );
    }
    const finalUpdatedInputs = currentInputs.map((input) =>
  ...
  const handleEditingInputChange = (id: string, field: keyof InputChannel, value: any) => {
    ...
    if (field === "type" && value !== updatedInput.type) {
      ...
      else if (value === "DI" || value === "Wireless" || value === "FX Return")
        updatedInput.phantom = false;
    }
  ...
  const handleCustomTypeKeyDown = async (
    ...
    if (e.key === "Enter") {
      ...
      if (detailKey) {
        ...
        // For direct fields like 'device'
        handleEditingInputChange(
          id,
          fieldTypeConstant === FIELD_TYPES.INPUT_DEVICE ? "device" : "type",
          value,
        );
      }
      ...
      if (!predefinedOptions.includes(value) && !currentSuggestions.includes(value)) {
        ...
        if (
          user &&
          !userCustomDevices.includes(value) &&
          fieldTypeConstant === FIELD_TYPES.INPUT_DEVICE
        ) {
          // Example for device
          try {
        ...
        } else if (user && fieldTypeConstant !== FIELD_TYPES.INPUT_DEVICE) {
          // Generalize for other types
          const userSpecificSuggestions =
            fieldTypeConstant === FIELD_TYPES.INPUT_CONN_ANALOG_SNAKE_TYPE
              ? userCustomAnalogSnakeTypes
              : fieldTypeConstant === FIELD_TYPES.INPUT_CONN_DIGITAL_SNAKE_TYPE
                ? userCustomDigitalSnakeTypes
                : fieldTypeConstant === FIELD_TYPES.INPUT_CONN_NETWORK_TYPE
                  ? userCustomNetworkTypes
                  : fieldTypeConstant === FIELD_TYPES.INPUT_CONN_CONSOLE_TYPE
                    ? userCustomConsoleTypes
                    : [];
  ...
  const getAllDevices = () =>
    getCombinedSuggestions(baseDeviceOptions, customDevices, userCustomDevices);
  ...
  const handleBulkAdd = () => {
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      let consoleInputNumber = bulkConnectionDetails.consoleInputNumber,
        networkPatch = bulkConnectionDetails.networkPatch,
        inputNumber = bulkConnectionDetails.inputNumber;
    ...
    const newEditModeInputs = { ...editModeInputs },
      newEditingInputs = { ...editingInputs };
  ...
  const handleBulkConnectionDetailChange = (detailKey: string, value: string) =>
    setBulkConnectionDetails((prev) => ({ ...prev, [detailKey]: value }));
  ...
  const getSuggestedDevicesForBulkType = () =>
    bulkType && deviceOptionsByType[bulkType] ? deviceOptionsByType[bulkType] : [];
                        ...
                        <div>
                          ...
                          <datalist id={`deviceOptions-${input.id}`}>
                            {(editingInput.type && deviceOptionsByType[editingInput.type]
                              ? getDeviceOptionsForType(editingInput.type)
                              : getAllDevices()
                            ).map((device, idx) => (
                              ...
                              <input
                                ...
                                onKeyDown={(e) =>
                                  handleCustomTypeKeyDown(
                                    e,
                                    input.id,
                                    FIELD_TYPES.INPUT_CONN_CONSOLE_TYPE,
                          ...
                          <>
                              ...
                              <input
                                ...
                                onKeyDown={(e) =>
                                  handleCustomTypeKeyDown(
                                    e,
                                    input.id,
                                    FIELD_TYPES.INPUT_CONN_ANALOG_SNAKE_TYPE,
                                    customSheetAnalogSnakeTypes,
                                    analogSnakeTypes,
                            ...
                            <div>
                              ...
                              <input
                                ...
                                className="w-full bg-gray-700 text-white border border-gray-600 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                placeholder="e.g., Avid S6L, DiGiCo SD12"
                                list={`consoleTypes-${input.id}-analog`}
                              />
                              <datalist id={`consoleTypes-${input.id}-analog`}>
                                {getAllConsoleTypes().map((type, idx) => (
                                  <option key={idx} value={type} />
                                ))}
                              </datalist>
                              ...
                              <input
                                ...
                                onKeyDown={(e) =>
                                  handleCustomTypeKeyDown(
                                    e,
                                    input.id,
                                    FIELD_TYPES.INPUT_CONN_DIGITAL_SNAKE_TYPE,
                                    customSheetDigitalSnakeTypes,
                                    digitalSnakeTypes,
                              ...
                              <input
                                ...
                                onKeyDown={(e) =>
                                  handleCustomTypeKeyDown(
                                    e,
                                    input.id,
                                    FIELD_TYPES.INPUT_CONN_NETWORK_TYPE,
