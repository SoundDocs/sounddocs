...
const PatchSheetOutputs: React.FC<PatchSheetOutputsProps> = ({ outputs, updateOutputs }) => {
  ...
  useEffect(() => {
    if (user) {
      fetchUserCustomSuggestions(user.id, "output_")
        .then((suggestionsMap) => {
          setUserCustomConsoleTypes(suggestionsMap[FIELD_TYPES.OUTPUT_SRC_CONSOLE_TYPE] || []);
          setUserCustomAnalogSnakeTypes(
            suggestionsMap[FIELD_TYPES.OUTPUT_SRC_ANALOG_SNAKE_TYPE] || [],
          );
          setUserCustomDigitalSnakeTypes(
            suggestionsMap[FIELD_TYPES.OUTPUT_SRC_DIGITAL_SNAKE_TYPE] || [],
          );
  ...
  useEffect(() => {
    ...
    outputs.forEach((output) => {
      ...
      setEditingOutputs((prev) => ({ ...prev, [output.id]: { ...output } }));
    });
  ...
  useEffect(() => {
    const destTypesSet = new Set<string>(),
      analogSnakesSet = new Set<string>(),
      digitalSnakesSet = new Set<string>(),
      networksSet = new Set<string>(),
      consolesSet = new Set<string>();
    outputs.forEach((output) => {
      if (output.destinationType && !destinationTypeOptions.includes(output.destinationType))
        destTypesSet.add(output.destinationType);
  ...
  useEffect(() => {
    ...
    outputs.forEach((output) => {
      if (editModeOutputs[output.id] === undefined)
        setEditModeOutputs((prev) => ({ ...prev, [output.id]: true }));
    });
  }, [outputs]);

  const updateParentOutputs = () => {
    const updatedOutputs = outputs.map((output) =>
      editModeOutputs[output.id] ? { ...editingOutputs[output.id] } : output,
    );
  ...
  const handleAddOutput = () => {
    const nextChannelNumber =
      outputs.length > 0
        ? Math.max(...outputs.map((o) => parseInt(o.channelNumber, 10) || 0)) + 1
        : 1;
  ...
  const handleDeleteOutput = (id: string) => {
    ...
    if (outputToDelete?.isStereo && outputToDelete?.stereoChannelNumber) {
      updatedOutputs = updatedOutputs.map((o) =>
        o.channelNumber === outputToDelete.stereoChannelNumber
          ? { ...o, isStereo: false, stereoChannelNumber: undefined }
          : o,
      );
    }
    updatedOutputs = updatedOutputs.map((o) =>
      o.stereoChannelNumber === outputToDelete?.channelNumber
        ? { ...o, isStereo: false, stereoChannelNumber: undefined }
        : o,
    );
    updateOutputs(updatedOutputs);
  ...
  const handleSaveOutput = (id: string) => {
    ...
    if (updatedOutput.isStereo && updatedOutput.stereoChannelNumber) {
      currentOutputs = currentOutputs.map((o) =>
        o.channelNumber === updatedOutput.stereoChannelNumber && o.id !== id
          ? { ...o, isStereo: true, stereoChannelNumber: updatedOutput.channelNumber }
          : o,
      );
    } else if (!updatedOutput.isStereo) {
      currentOutputs = currentOutputs.map((o) =>
        o.stereoChannelNumber === updatedOutput.channelNumber
          ? { ...o, isStereo: false, stereoChannelNumber: undefined }
          : o,
      );
    }
  ...
  const getAvailableChannelsForStereo = (
    ...
    return outputs
      .filter(
        (o) =>
          o.id !== currentChannelId &&
          (!o.isStereo || o.stereoChannelNumber === currentChannelNumber),
      )
      .map((o) => ({
        channelNumber: o.channelNumber,
        name: o.name || `Channel ${o.channelNumber}`,
      }));
  };
  ...
  const handleCustomTypeKeyDown = async (
    ...
    id: string,
    fieldTypeConstant: string,
    currentSuggestions: string[],
    predefinedOptions: string[],
    setSheetCustomState: React.Dispatch<React.SetStateAction<string[]>>,
    setUserCustomState: React.Dispatch<React.SetStateAction<string[]>>,
    detailKey?: string,
    directFieldKey?: keyof OutputChannel,
  ) => {
    if (e.key === "Enter") {
      e.preventDefault();
      ...
      if (!predefinedOptions.includes(value) && !currentSuggestions.includes(value)) {
        ...
        switch (fieldTypeConstant) {
          case FIELD_TYPES.OUTPUT_SRC_CONSOLE_TYPE:
            userSpecificSuggestions = userCustomConsoleTypes;
            break;
          case FIELD_TYPES.OUTPUT_SRC_ANALOG_SNAKE_TYPE:
            userSpecificSuggestions = userCustomAnalogSnakeTypes;
            break;
          case FIELD_TYPES.OUTPUT_SRC_DIGITAL_SNAKE_TYPE:
            userSpecificSuggestions = userCustomDigitalSnakeTypes;
            break;
          case FIELD_TYPES.OUTPUT_SRC_NETWORK_TYPE:
            userSpecificSuggestions = userCustomNetworkTypes;
            break;
          case FIELD_TYPES.OUTPUT_DESTINATION_TYPE:
            userSpecificSuggestions = userCustomDestinationTypes;
            break;
          case FIELD_TYPES.OUTPUT_DESTINATION_GEAR:
            userSpecificSuggestions = userCustomDestinationGear;
            break;
        }
        ...
        if (user && !userSpecificSuggestions.includes(value)) {
          try {
            ...
            console.error("Failed to save custom suggestion:", err);
          }
        }
      }
    }
  ...
  const handleSelectDestinationType = (id: string, value: string) => {
    ...
    if (
      ...
      !customSheetDestinationTypes.includes(value)
    ) {
    ...
    if (
      ...
      value.trim() &&
      !destinationTypeOptions.includes(value) &&
      !userCustomDestinationTypes.includes(value)
    ) {
  ...
  const toggleDestinationTypeOptions = (id: string) =>
    setShowDestinationTypeOptions((prev) => ({ ...prev, [id]: !prev[id] }));

  const getCombinedSuggestions = (base: string[], sheetCustom: string[], userCustom: string[]) =>
    Array.from(new Set([...base, ...sheetCustom, ...userCustom]));

  const getAllDestinationTypes = () =>
    getCombinedSuggestions(
      destinationTypeOptions,
      customSheetDestinationTypes,
      userCustomDestinationTypes,
    );
  const getAllAnalogSnakeTypes = () =>
    getCombinedSuggestions(
      analogSnakeTypes,
      customSheetAnalogSnakeTypes,
      userCustomAnalogSnakeTypes,
    );
  const getAllDigitalSnakeTypes = () =>
    getCombinedSuggestions(
      digitalSnakeTypes,
      customSheetDigitalSnakeTypes,
      userCustomDigitalSnakeTypes,
    );
  const getAllNetworkTypes = () =>
    getCombinedSuggestions(networkTypeOptions, customSheetNetworkTypes, userCustomNetworkTypes);
  const getAllConsoleTypes = () =>
    getCombinedSuggestions(consoleTypeOptions, customSheetConsoleTypes, userCustomConsoleTypes);
  // For Destination Gear, there are no predefined options, so it's just sheet custom (if any) + user custom
  const getAllDestinationGearSuggestions = () =>
    Array.from(new Set([...userCustomDestinationGear])); // Assuming no sheet-local for gear yet
  ...
  const handleBulkAdd = () => {
    const quantity = parseInt(String(bulkQuantity), 10),
      startNum = parseInt(String(bulkStartChannel), 10);
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      let consoleOutputNumber = bulkSourceDetails.consoleOutputNumber,
        networkPatch = bulkSourceDetails.networkPatch,
        outputNumber = bulkSourceDetails.outputNumber;
    ...
    const newEditModeOutputs = { ...editModeOutputs },
      newEditingOutputs = { ...editingOutputs };
    newOutputsArr.forEach((o) => {
      newEditModeOutputs[o.id] = true;
      newEditingOutputs[o.id] = { ...o };
    });
  ...
  const handleBulkSourceDetailChange = (detailKey: string, value: string) =>
    setBulkSourceDetails((prev) => ({ ...prev, [detailKey]: value }));
                          ...
                          <>
                            <div>
                              ...
                              <input
                                ...
                                onKeyDown={(e) =>
                                  handleCustomTypeKeyDown(
                                    e,
                                    output.id,
                                    FIELD_TYPES.OUTPUT_SRC_CONSOLE_TYPE,
                                    customSheetConsoleTypes,
                                    consoleTypeOptions,
                                    setCustomSheetConsoleTypes,
                                    setUserCustomConsoleTypes,
                                    "consoleType",
                                  )
                                }
                                ...
                                list={`consoleTypes-output-${output.id}`}
                              />
                              <datalist id={`consoleTypes-output-${output.id}`}>
                          ...
                          <>
                              ...
                              <input
                                ...
                                onKeyDown={(e) =>
                                  handleCustomTypeKeyDown(
                                    e,
                                    output.id,
                                    FIELD_TYPES.OUTPUT_SRC_ANALOG_SNAKE_TYPE,
                                    customSheetAnalogSnakeTypes,
                                    analogSnakeTypes,
                                    setCustomSheetAnalogSnakeTypes,
                                    setUserCustomAnalogSnakeTypes,
                                    "snakeType",
                                  )
                                }
                                ...
                                list={`analogSnakeTypes-output-${output.id}`}
                              />
                              <datalist id={`analogSnakeTypes-output-${output.id}`}>
                            ...
                            <div>
                              ...
                              <input
                                ...
                                className="w-full bg-gray-700 text-white border border-gray-600 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                placeholder="e.g., Avid S6L"
                                list={`consoleTypes-output-${output.id}-analog`}
                              />
                              <datalist id={`consoleTypes-output-${output.id}-analog`}>
                                {getAllConsoleTypes().map((type, idx) => (
                                  <option key={idx} value={type} />
                                ))}
                              </datalist>
                              ...
                              <input
                                ...
                                onKeyDown={(e) =>
                                  handleCustomTypeKeyDown(
                                    e,
                                    output.id,
                                    FIELD_TYPES.OUTPUT_SRC_DIGITAL_SNAKE_TYPE,
                                    customSheetDigitalSnakeTypes,
                                    digitalSnakeTypes,
                                    setCustomSheetDigitalSnakeTypes,
                                    setUserCustomDigitalSnakeTypes,
                                    "snakeType",
                                  )
                                }
                                ...
                                list={`digitalSnakeTypes-output-${output.id}`}
                              />
                              <datalist id={`digitalSnakeTypes-output-${output.id}`}>
                              ...
                              <input
                                ...
                                onKeyDown={(e) =>
                                  handleCustomTypeKeyDown(
                                    e,
                                    output.id,
                                    FIELD_TYPES.OUTPUT_SRC_NETWORK_TYPE,
                                    customSheetNetworkTypes,
                                    networkTypeOptions,
                                    setCustomSheetNetworkTypes,
                                    setUserCustomNetworkTypes,
                                    "networkType",
                                  )
                                }
                                ...
                                list={`networkTypes-output-${output.id}`}
                              />
                              <datalist id={`networkTypes-output-${output.id}`}>
                            ...
                            <input
                              ...
                              onKeyDown={(e) =>
                                handleCustomTypeKeyDown(
                                  e,
                                  output.id,
                                  FIELD_TYPES.OUTPUT_DESTINATION_GEAR,
                                  [],
                                  [],
                                  () => {},
                                  setUserCustomDestinationGear,
                              ...
                              list={`destinationGear-output-${output.id}`}
                            />
                            <datalist id={`destinationGear-output-${output.id}`}>
                              {getAllDestinationGearSuggestions().map((gear, idx) => (
