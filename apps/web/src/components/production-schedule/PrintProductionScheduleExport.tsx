...
const PrintProductionScheduleExport = forwardRef<
  HTMLDivElement,
  PrintProductionScheduleExportProps
>(({ schedule }, ref) => {
  console.log(
    "[PrintExport] Received schedule prop:",
  ...
  console.log(
    "[PrintExport] Extracted detailed_schedule_items (after robust check):",
  ...
  const getCrewNames = (crewIds: string[], crewKeyData: CrewKeyItem[]): string => {
    ...
    return crewIds.map((id) => crewKeyData.find((c) => c.id === id)?.name || "Unknown").join(", ");
  };

  const groupAndSortDetailedItems = (items: DetailedScheduleItem[]) => {
    console.log(
      "[PrintExport] groupAndSortDetailedItems input:",
  ...
  console.log(
    "[PrintExport] Rendering with groupedDetailedScheduleItems count:",
      ...
      <div style={{ marginBottom: "20px", fontSize: "13px" }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <tbody>
            {info.event_name && (
              <tr>
                <td style={{ padding: "4px 0", fontWeight: "bold", width: "150px" }}>Event:</td>
                <td>{info.event_name}</td>
              </tr>
            )}
            {info.job_number && (
              <tr>
                <td style={{ padding: "4px 0", fontWeight: "bold" }}>Job #:</td>
        ...
        <div style={{ flex: 1 }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <tbody>
              {info.venue && (
                <tr>
                  <td style={{ padding: "4px 0", fontWeight: "bold", width: "150px" }}>Venue:</td>
                  <td>{info.venue}</td>
                </tr>
              )}
              {info.project_manager && (
                <tr>
                  <td style={{ padding: "4px 0", fontWeight: "bold" }}>Project Manager:</td>
                  <td>{info.project_manager}</td>
                </tr>
              )}
              {info.production_manager && (
                <tr>
                  <td style={{ padding: "4px 0", fontWeight: "bold" }}>Production Manager:</td>
                  <td>{info.production_manager}</td>
                </tr>
              )}
              {info.account_manager && (
                <tr>
                  <td style={{ padding: "4px 0", fontWeight: "bold" }}>Account Manager:</td>
                  <td>{info.account_manager}</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div style={{ flex: 1 }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <tbody>
                {info.contact_name && (
                  <tr>
                    <td style={{ padding: "4px 0", fontWeight: "bold", width: "120px" }}>Name:</td>
                    ...
                    {itemsInGroup.map((item, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {getCrewNames(item.assigned_crew_ids, crewKey)}
                          </td>
                        </tr>
                      );
                    })}
      ...
      {sortedLaborScheduleItems.length > 0 && (
        <div style={{ marginBottom: "20px", pageBreakInside: "avoid" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
            <thead>
              <tr style={{ backgroundColor: "#f0f0f0", borderBottom: "1px solid #ccc" }}>
                <th style={{ padding: "6px", textAlign: "left", fontWeight: "bold", width: "20%" }}>
                  Name
                </th>
                <th style={{ padding: "6px", textAlign: "left", fontWeight: "bold", width: "20%" }}>
                  Position
                </th>
                <th style={{ padding: "6px", textAlign: "left", fontWeight: "bold", width: "15%" }}>
                  Date
                </th>
                <th style={{ padding: "6px", textAlign: "left", fontWeight: "bold", width: "10%" }}>
                  Time In
                </th>
                <th style={{ padding: "6px", textAlign: "left", fontWeight: "bold", width: "10%" }}>
                  Time Out
                </th>
                <th style={{ padding: "6px", textAlign: "left", fontWeight: "bold", width: "25%" }}>
                  Notes
                </th>
              </tr>
            </thead>
              ...
              {sortedLaborScheduleItems.map((item, index) => {
                ...
                return (
                  <React.Fragment key={item.id}>
                    ...
                    >
                      <td style={{ padding: "6px", verticalAlign: "top", fontWeight: "500" }}>
                        {item.name || "-"}
                      </td>
                      <td style={{ padding: "6px", verticalAlign: "top" }}>
                        {item.position || "-"}
