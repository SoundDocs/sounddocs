...
const DetailedScheduleItemRow: React.FC<{
  ...
  const handleInputChange = (
    field: keyof Omit<DetailedScheduleItem, "assigned_crew_ids">,
    value: string,
  ) => {
...
const ProductionScheduleDetail: React.FC<ProductionScheduleDetailProps> = ({
  detailedItems,
  ...
  const handleDuplicateDayGroup = (dateKey: string) => {
    const itemsToDuplicate = detailedItems.filter(
              ...
              {editingDateKey === dateKey ? (
                ...
                <>
                  ...
                  {dateKey !== "No Date Assigned" && (
                    <button
                      onClick={() => handleStartEditDate(dateKey)}
