...
const ProductionScheduleExport = forwardRef<HTMLDivElement, ProductionScheduleExportProps>(
  ({ schedule, forDisplay }, ref) => {
    ...
    console.log(
      "[Export] Extracted detailed_schedule_items (after robust check):",
      JSON.parse(JSON.stringify(detailedScheduleItemsToUse)),
    );
    ...
    const formatDate = (dateString?: string | null, options?: Intl.DateTimeFormatOptions) => {
      ...
      if (/^\d{4}-\d{2}-\d{2}$/.test(dateString) && !dateString.includes("T")) {
        try {
          ...
          if (
            localDateObject.getFullYear() === year &&
          ...
          {crewIds.map((id) => {
            ...
            if (!crewMember)
              return (
                <span
                  key={id}
                  className="text-xs bg-gray-600 text-gray-300 px-1.5 py-0.5 rounded-full"
                >
                  Unknown
                </span>
              );
    ...
    const groupAndSortDetailedItems = (items: DetailedScheduleItem[]) => {
      ...
      console.log(
        "[Export] groupAndSortDetailedItems output (grouped and sorted):",
    ...
    console.log(
      "[Export] Rendering with groupedDetailedScheduleItems count:",
      groupedDetailedScheduleItems.length,
    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
              {info.event_name && (
                <div>
                  <strong className="text-gray-400 block">Event:</strong> {info.event_name}
                </div>
              )}
              {info.job_number && (
                <div>
                  <strong className="text-gray-400 block">Job #:</strong> {info.job_number}
                </div>
              )}
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
              {info.venue && (
                <div>
                  <strong className="text-gray-400 block">Venue:</strong> {info.venue}
                </div>
              )}
              {info.project_manager && (
                <div>
                  <strong className="text-gray-400 block">Project Manager:</strong>{" "}
                  {info.project_manager}
                </div>
              )}
              {info.production_manager && (
                <div>
                  <strong className="text-gray-400 block">Production Manager:</strong>{" "}
                  {info.production_manager}
                </div>
              )}
              {info.account_manager && (
                <div>
                  <strong className="text-gray-400 block">Account Manager:</strong>{" "}
                  {info.account_manager}
                </div>
              )}
            </div>
          ...
          {(info.contact_name || info.contact_email || info.contact_phone) && (
            ...
            >
              ...
              <div className="grid grid-cols-1 gap-y-3 text-sm">
                {info.contact_name && (
                  <div>
                    <strong className="text-gray-400 block">Name:</strong> {info.contact_name}
                  </div>
                )}
                {info.contact_email && (
                  <div>
                    <strong className="text-gray-400 block">Email:</strong>{" "}
                    <a
                ...
                {info.contact_phone && (
                  <div>
                    <strong className="text-gray-400 block">Phone:</strong>{" "}
                    <a
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  ...
                  >
                    <th className="py-3 px-4 text-indigo-400 font-medium align-middle text-sm w-[8%]">
                      Start
                    </th>
                    <th className="py-3 px-4 text-indigo-400 font-medium align-middle text-sm w-[8%]">
                      End
                    </th>
                    <th className="py-3 px-4 text-indigo-400 font-medium align-middle text-sm w-[30%]">
                      Activity
                    </th>
                    <th className="py-3 px-4 text-indigo-400 font-medium align-middle text-sm w-[30%]">
                      Notes
                    </th>
                    <th className="py-3 px-4 text-indigo-400 font-medium align-middle text-sm w-[24%]">
                      Crew
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {groupedDetailedScheduleItems.map(([dateKey, itemsInGroup], groupIndex) => {
                    console.log(
                      `[Export] Rendering group ${groupIndex} for date: ${dateKey}, items count: ${itemsInGroup.length}`,
                    );
                    return (
                      <React.Fragment key={dateKey}>
                        ...
                        >
                          <td
                            colSpan={5}
                            className="py-2 px-4 text-white font-semibold text-sm flex items-center whitespace-nowrap"
                          >
                            <CalendarDays
                              size={16}
                              className="mr-2 text-indigo-300 flex-shrink-0"
                            />
                            {formatDate(dateKey, {
                              weekday: "long",
                              year: "numeric",
                        ...
                        {itemsInGroup.map((item, index) => {
                          console.log(
                            `[Export] Rendering item ${index} in group ${groupIndex}:`,
                          ...
                          return (
                            ...
                            >
                              <td className="py-3 px-4 text-white align-top text-sm">
                              ...
                              <td
                                className="py-3 px-4 text-white align-top text-sm"
                                style={{ whiteSpace: "pre-wrap", wordBreak: "break-word" }}
                              >
                                {item.activity || "-"}
                              </td>
                              <td
                                className="py-3 px-4 text-gray-300 align-top text-sm"
                                style={{ whiteSpace: "pre-wrap", wordBreak: "break-word" }}
                              >
                                {item.notes || "-"}
                              </td>
                              <td className="py-3 px-4 text-white align-top text-sm">
                                {getCrewDisplay(item.assigned_crew_ids, crewKey)}
                              </td>
                            </tr>
                          );
                        })}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  ...
                  >
                    <th className="py-3 px-4 text-indigo-400 font-medium align-middle text-sm">
                      Name
                    </th>
                    <th className="py-3 px-4 text-indigo-400 font-medium align-middle text-sm">
                      Position
                ...
                <tbody>
                  {sortedLaborScheduleItems.map((item, index) => {
                    const itemDayDisplay = formatDate(item.date, {
                      weekday: "long",
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    ...
                    const showDayHeader =
                      item.date &&
                      itemDayDisplay !== "N/A" &&
                      itemDayDisplay !== currentLaborDayFormatted;
                    if (showDayHeader) {
                    ...
                    return (
                      <React.Fragment key={item.id}>
                        ...
                        >
                          <td className="py-3 px-4 text-white align-middle text-sm font-medium">
                            {item.name || "-"}
                          </td>
                          ...
                          <td
                            className="py-3 px-4 text-gray-300 align-middle text-sm"
                            style={{ whiteSpace: "pre-wrap", wordBreak: "break-word" }}
                          >
                            {item.notes || "-"}
                          </td>
