    ...
    <div className="bg-gray-800 p-4 md:p-6 rounded-xl shadow-lg mb-6">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="md:w-1/3 flex flex-col items-center">
          ...
          {!processingPhoto && !entry.photo_url && (
            <p className="text-gray-400 text-xs mt-2 text-center">Drag & drop or click to upload</p>
          )}
        </div>
        ...
        <div className="md:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label
              htmlFor={`character_names_${entry.id}`}
              className="block text-sm font-medium text-gray-300 mb-1 flex items-center"
            >
              <CharacterIcon size={14} className="mr-1 text-gray-400" />
              Character Name(s)
            </label>
            <input
              type="text"
              id={`character_names_${entry.id}`}
              value={entry.character_names}
              onChange={(e) => handleInputChange("character_names", e.target.value)}
              className="w-full bg-gray-700 border border-gray-600 text-white rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm"
              placeholder="e.g., Hamlet, Ghost"
            />
          </div>
          <div>
            <label
              htmlFor={`element_channel_number_${entry.id}`}
              className="block text-sm font-medium text-gray-300 mb-1 flex items-center"
            >
              <Mic size={14} className="mr-1 text-gray-400" />
              Element/Channel No.
            </label>
            <input
              type="text"
              id={`element_channel_number_${entry.id}`}
          ...
          <div>
            <label
              htmlFor={`element_color_${entry.id}`}
              className="block text-sm font-medium text-gray-300 mb-1 flex items-center"
            >
              <Palette size={14} className="mr-1 text-gray-400" />
              Element Color
            </label>
            <input
              type="text"
              id={`element_color_${entry.id}`}
              value={entry.element_color}
              onChange={(e) => handleInputChange("element_color", e.target.value)}
              className="w-full bg-gray-700 border border-gray-600 text-white rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm"
              placeholder="e.g., Red"
            />
          </div>
          <div>
            <label
              htmlFor={`transmitter_location_${entry.id}`}
            ...
            <input
              type="text"
              id={`transmitter_location_${entry.id}`}
              value={entry.transmitter_location}
              onChange={(e) => handleInputChange("transmitter_location", e.target.value)}
              className="w-full bg-gray-700 border border-gray-600 text-white rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm"
              placeholder="e.g., Waist pack R, In wig"
            />
          </div>
          <div>
            <label
              htmlFor={`element_location_${entry.id}`}
              className="block text-sm font-medium text-gray-300 mb-1"
            >
              Element Location
            </label>
            <input
              type="text"
              id={`element_location_${entry.id}`}
              value={entry.element_location}
              onChange={(e) => handleInputChange("element_location", e.target.value)}
              className="w-full bg-gray-700 border border-gray-600 text-white rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm"
              placeholder="e.g., Forehead, Cheek L"
            />
          </div>
          <div>
            <label
              htmlFor={`backup_element_${entry.id}`}
              className="block text-sm font-medium text-gray-300 mb-1"
            >
              Backup Element
            </label>
          ...
          <div>
            <label
              htmlFor={`scene_numbers_${entry.id}`}
              className="block text-sm font-medium text-gray-300 mb-1 flex items-center"
            >
            ...
            <input
              type="text"
              id={`scene_numbers_${entry.id}`}
              value={entry.scene_numbers}
              onChange={(e) => handleInputChange("scene_numbers", e.target.value)}
              className="w-full bg-gray-700 border border-gray-600 text-white rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm"
          ...
          <div className="sm:col-span-2">
            <label
              htmlFor={`costume_notes_${entry.id}`}
              className="block text-sm font-medium text-gray-300 mb-1 flex items-center"
            >
              <Shirt size={14} className="mr-1 text-gray-400" />
              Costume Notes
            </label>
            <textarea
              id={`costume_notes_${entry.id}`}
              value={entry.costume_notes}
              onChange={(e) => handleInputChange("costume_notes", e.target.value)}
              rows={2}
              className="w-full bg-gray-700 border border-gray-600 text-white rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm"
              placeholder="e.g., Belt available, quick change after Sc 3"
            />
          </div>
          <div className="sm:col-span-2">
            <label
              htmlFor={`wig_hair_notes_${entry.id}`}
              className="block text-sm font-medium text-gray-300 mb-1 flex items-center"
            >
            ...
            <textarea
              id={`wig_hair_notes_${entry.id}`}
              value={entry.wig_hair_notes}
              onChange={(e) => handleInputChange("wig_hair_notes", e.target.value)}
              rows={2}
              className="w-full bg-gray-700 border border-gray-600 text-white rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm"
              placeholder="e.g., Mic in wig, careful with hairspray"
            />
          </div>
