  ...
  const handleDuplicateMicPlot = async (micPlot: CorporateMicPlot) => {
    ...
    try {
      ...
      if (newMicPlotData && newMicPlotData[0]) {
        ...
        const sortFn = (a: CorporateMicPlot, b: CorporateMicPlot) =>
          new Date(b.last_edited || b.created_at).getTime() -
          new Date(a.last_edited || a.created_at).getTime();
  ...
  if (screenSize === "mobile" || screenSize === "tablet") {
    return (
      <MobileScreenWarning
        ...
        description="Corporate Mic Plot management requires a larger screen. Please use a desktop or laptop."
      />
    );
  }
    ...
    <div className="min-h-screen bg-gray-900 flex flex-col">
      ...
      <main className="flex-grow container mx-auto px-4 py-12 mt-12">
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={() =>
              navigate("/corporate-mic-plot/new", { state: { from: "/all-corporate-mic-plots" } })
            }
        ...
        <div className="bg-gray-800 rounded-xl shadow-lg overflow-hidden mb-8">
                ...
                {filteredMicPlots.length > 0 ? (
                    ...
                    <tr key={plot.id} className="hover:bg-gray-750 transition-colors">
                      <td className="py-4 px-6">
                        <div className="flex items-center">
                          <Mic className="h-5 w-5 text-indigo-400 mr-3 flex-shrink-0" />
                  ...
                  <tr>
                    <td colSpan={4} className="py-12 text-center text-gray-400">
                      {searchTerm
                        ? "No corporate mic plots match your search criteria"
                        : "You haven't created any corporate mic plots yet"}
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          {filteredMicPlots.length > 0 && (
            <div className="p-4 border-t border-gray-700 text-gray-400 text-sm">
              Showing {filteredMicPlots.length} of {micPlots.length} corporate mic plots
            </div>
          )}
        </div>
      </main>
      ...
      {showShareModal && selectedShareMicPlot && (
        <ShareModal
          ...
          resourceType="corporate_mic_plot"
