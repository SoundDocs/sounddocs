...
const UpdatePasswordPage: React.FC = () => {
    ...
    const timer = setTimeout(() => {
      if (checkingToken) {
        supabase.auth.getSession().then(({ data: { session: currentSession } }) => {
          if (currentSession && currentSession.user.aud === "authenticated") {
            // Check if it's a valid user session
            setSession(currentSession);
          } else if (!session) {
            // Only set error if no session was found by onAuthStateChange either
            setMessage({
              type: "error",
  ...
  if (
    ...
    (!message ||
      ...
      !message.text.includes("You can now update your password."))
  ) {
    return (
      <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center text-white p-4">
