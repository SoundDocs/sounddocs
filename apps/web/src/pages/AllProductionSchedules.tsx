...
const AllProductionSchedules: React.FC = () => {
  ...
  const [currentExportScheduleData, setCurrentExportScheduleData] =
    useState<ScheduleForExport | null>(null);
  const [isExporting, setIsExporting] = useState(false);
  const [duplicatingId, setDuplicatingId] = useState<string | null>(null);

  const [showShareModal, setShowShareModal] = useState(false);
  const [selectedShareSchedule, setSelectedShareSchedule] =
    useState<ProductionScheduleSummary | null>(null);
  ...
  const fetchFullScheduleDataForExport = async (
    scheduleId: string,
  ): Promise<FullProductionScheduleData | null> => {
    ...
    <div className="min-h-screen bg-gray-900 flex flex-col">
      ...
      <main className="flex-grow container mx-auto px-4 py-12 mt-12">
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={() =>
              navigate("/production-schedule/new", { state: { from: "/all-production-schedules" } })
            }
                ...
                <button
                  onClick={() =>
                    navigate("/production-schedule/new", {
                      state: { from: "/all-production-schedules" },
                    })
                  }
                    ...
                    <tr key={schedule.id} className="hover:bg-gray-750 transition-colors">
                      <td className="py-4 px-6">
                        <div className="flex items-center">
                          ...
                          <span
                            ...
                            onClick={() =>
                              navigate(`/production-schedule/${schedule.id}`, {
                                state: { from: "/all-production-schedules" },
                              })
                            }
                          >
                      ...
                      <td className="py-4 px-6">
                        <div className="flex justify-end space-x-2">
                          ...
                          >
                            <Copy
                              className={`h-5 w-5 ${duplicatingId === schedule.id ? "animate-pulse" : ""}`}
                            />
                          </button>
                          <button
                            ...
                            disabled={
                              (isExporting && exportScheduleId === schedule.id) ||
                              duplicatingId === schedule.id
                            }
                          >
                            <Download
                              className={`h-5 w-5 ${isExporting && exportScheduleId === schedule.id ? "animate-pulse" : ""}`}
                            />
                          </button>
                          <button
                            onClick={() =>
      ...
      <ExportModal
        ...
        onExportPrintFriendly={() =>
          exportScheduleId && prepareAndExecuteExport(exportScheduleId, "print")
        }
