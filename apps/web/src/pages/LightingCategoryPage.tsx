...
const LightingCategoryPage: React.FC = () => {
  ...
  // Lumens & Lux
  const numLumens = useMemo(
    () => (typeof lumens === "number" && lumens > 0 ? lumens : NaN),
    [lumens],
  );
  const numAreaM2 = useMemo(
    () => (typeof areaM2 === "number" && areaM2 > 0 ? areaM2 : NaN),
    [areaM2],
  );
  const numLuxForLumens = useMemo(
    () => (typeof luxForLumens === "number" && luxForLumens > 0 ? luxForLumens : NaN),
    [luxForLumens],
  );

  const calculatedLux = useMemo(
    () => (!isNaN(numLumens) && !isNaN(numAreaM2) ? numLumens / numAreaM2 : NaN),
    [numLumens, numAreaM2],
  );
  const calculatedLumens = useMemo(
    () => (!isNaN(numLuxForLumens) && !isNaN(numAreaM2) ? numLuxForLumens * numAreaM2 : NaN),
    [numLuxForLumens, numAreaM2],
  );

  // Lux / Foot-candle Converter
  const numLuxToFc = useMemo(
    () => (typeof luxToFc === "number" && luxToFc >= 0 ? luxToFc : NaN),
    [luxToFc],
  );
  const numFcToLux = useMemo(
    () => (typeof fcToLux === "number" && fcToLux >= 0 ? fcToLux : NaN),
    [fcToLux],
  );
  const LUX_TO_FC_FACTOR = 10.7639;
  const calculatedFc = useMemo(
    () => (!isNaN(numLuxToFc) ? numLuxToFc / LUX_TO_FC_FACTOR : NaN),
    [numLuxToFc],
  );
  const calculatedLuxFromFc = useMemo(
    () => (!isNaN(numFcToLux) ? numFcToLux * LUX_TO_FC_FACTOR : NaN),
    [numFcToLux],
  );

  // Inverse Square Law
  const numInitialIlluminance = useMemo(
    () =>
      typeof initialIlluminance === "number" && initialIlluminance > 0 ? initialIlluminance : NaN,
    [initialIlluminance],
  );
  const numDistance1 = useMemo(
    () => (typeof distance1 === "number" && distance1 > 0 ? distance1 : NaN),
    [distance1],
  );
  const numDistance2 = useMemo(
    () => (typeof distance2 === "number" && distance2 > 0 ? distance2 : NaN),
    [distance2],
  );
  ...
  // Beam Diameter
  const numBeamAngle = useMemo(
    () => (typeof beamAngle === "number" && beamAngle > 0 && beamAngle < 180 ? beamAngle : NaN),
    [beamAngle],
  );
  const numBeamDistance = useMemo(
    () => (typeof beamDistance === "number" && beamDistance > 0 ? beamDistance : NaN),
    [beamDistance],
  );
  ...
  // DMX Channel Offset
  const numDmxStartAddress = useMemo(
    () =>
      typeof dmxStartAddress === "number" && dmxStartAddress >= 1 && dmxStartAddress <= 512
        ? dmxStartAddress
        : NaN,
    [dmxStartAddress],
  );
  const numDmxChannelOffset = useMemo(
    () => (typeof dmxChannelOffset === "number" && dmxChannelOffset >= 0 ? dmxChannelOffset : NaN),
    [dmxChannelOffset],
  );
  ...
  // Power Load
  const numPowerPerFixture = useMemo(
    () => (typeof powerPerFixture === "number" && powerPerFixture > 0 ? powerPerFixture : NaN),
    [powerPerFixture],
  );
  const numNumberOfFixtures = useMemo(
    () =>
      typeof numberOfFixtures === "number" &&
      numberOfFixtures > 0 &&
      Number.isInteger(numberOfFixtures)
        ? numberOfFixtures
        : NaN,
    [numberOfFixtures],
  );
  const totalPowerLoad = useMemo(
    () =>
      !isNaN(numPowerPerFixture) && !isNaN(numNumberOfFixtures)
        ? numPowerPerFixture * numNumberOfFixtures
        : NaN,
    [numPowerPerFixture, numNumberOfFixtures],
  );
  const totalAmps120V = useMemo(
    () => (!isNaN(totalPowerLoad) ? totalPowerLoad / 120 : NaN),
    [totalPowerLoad],
  );
  const totalAmps230V = useMemo(
    () => (!isNaN(totalPowerLoad) ? totalPowerLoad / 230 : NaN),
    [totalPowerLoad],
  );

  // CCT Conversion
  const numKelvinToMired = useMemo(
    () => (typeof kelvinToMired === "number" && kelvinToMired > 0 ? kelvinToMired : NaN),
    [kelvinToMired],
  );
  const numMiredToKelvin = useMemo(
    () => (typeof miredToKelvin === "number" && miredToKelvin > 0 ? miredToKelvin : NaN),
    [miredToKelvin],
  );
  const calculatedMired = useMemo(
    () => (!isNaN(numKelvinToMired) ? 1000000 / numKelvinToMired : NaN),
    [numKelvinToMired],
  );
  const calculatedKelvinFromMired = useMemo(
    () => (!isNaN(numMiredToKelvin) ? 1000000 / numMiredToKelvin : NaN),
    [numMiredToKelvin],
  );

  // Luminous Efficacy
  const numEfficacyLumens = useMemo(
    () => (typeof efficacyLumens === "number" && efficacyLumens > 0 ? efficacyLumens : NaN),
    [efficacyLumens],
  );
  const numEfficacyWatts = useMemo(
    () => (typeof efficacyWatts === "number" && efficacyWatts > 0 ? efficacyWatts : NaN),
    [efficacyWatts],
  );
  const calculatedEfficacy = useMemo(
    () =>
      !isNaN(numEfficacyLumens) && !isNaN(numEfficacyWatts)
        ? numEfficacyLumens / numEfficacyWatts
        : NaN,
    [numEfficacyLumens, numEfficacyWatts],
  );

  // Candela to Lumens
  const numCandelaValue = useMemo(
    () => (typeof candelaValue === "number" && candelaValue > 0 ? candelaValue : NaN),
    [candelaValue],
  );
  const numCandelaBeamAngle = useMemo(
    () =>
      typeof candelaBeamAngle === "number" && candelaBeamAngle > 0 && candelaBeamAngle < 360
        ? candelaBeamAngle
        : NaN,
    [candelaBeamAngle],
  );
  ...
  // Throw Distance for Min Lux
  const numThrowCandela = useMemo(
    () => (typeof throwCandela === "number" && throwCandela > 0 ? throwCandela : NaN),
    [throwCandela],
  );
  const numMinLuxTarget = useMemo(
    () => (typeof minLuxTarget === "number" && minLuxTarget > 0 ? minLuxTarget : NaN),
    [minLuxTarget],
  );
  ...
  // Lighting Energy Density
  const numTotalPowerWatts = useMemo(
    () => (typeof totalPowerWatts === "number" && totalPowerWatts > 0 ? totalPowerWatts : NaN),
    [totalPowerWatts],
  );
  const numTotalAreaSq = useMemo(
    () => (typeof totalAreaSq === "number" && totalAreaSq > 0 ? totalAreaSq : NaN),
    [totalAreaSq],
  );
  ...
  const numLensFocalLength = useMemo(
    () => (typeof lensFocalLength === "number" && lensFocalLength > 0 ? lensFocalLength : NaN),
    [lensFocalLength],
  );
  const numProjectionDistance = useMemo(
    () =>
      typeof projectionDistance === "number" && projectionDistance > 0 ? projectionDistance : NaN,
    [projectionDistance],
  );
    ...
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!isNaN(numValue)) {
        if (!allowZero && numValue <= 0 && value !== "0.") {
          ...
          /* Do nothing for non-integers if integer expected */
        } else {
          setter(numValue);
        }
      } else if (value.endsWith(".") && allowFloat && !isInteger) {
        setter(value);
      }
    };
  ...
  const handleInitialIlluminanceChange = handleSimpleNumberChange(
    setInitialIlluminance,
    false,
    true,
  );
  ...
  const handleDmxStartAddressChange = handleSimpleNumberChange(
    setDmxStartAddress,
    false,
    false,
    true,
  );
  const handleDmxChannelOffsetChange = handleSimpleNumberChange(
    setDmxChannelOffset,
    true,
    false,
    true,
  ...
  const handleNumberOfFixturesChange = handleSimpleNumberChange(
    setNumberOfFixtures,
    false,
    false,
    true,
  ...
  const handleProjectionDistanceChange = handleSimpleNumberChange(
  ...
  const renderCalcGroup = (
    ...
    <div className="mb-8 bg-gray-800 p-6 sm:p-8 rounded-xl shadow-xl border border-yellow-700/30">
      ...
      {condition ? (
        ...
        <p className="text-orange-400 text-center bg-orange-900/30 p-3 rounded-md mt-4">
          {errorMsg}
        </p>
      )}
    </div>
      ...
      <main className="flex-grow container mx-auto px-4 py-16 sm:py-24">
        ...
        <section id="lumens-lux-section" className="mb-12">
          ...
          {renderCalcGroup(
            ...
            [{ label: "Illuminance", value: formatNumber(calculatedLux), unit: "lux" }].map(
          ...
          {renderCalcGroup(
            ...
            [{ label: "Total Lumens", value: formatNumber(calculatedLumens), unit: "lm" }].map(
              (item) => renderOutputItem(item.label, item.value, item.unit),
            ),
        ...
        <section id="lux-fc-converter-section" className="mb-12">
          ...
          {renderCalcGroup(
            ...
            [{ label: "Illuminance", value: formatNumber(calculatedFc, 3), unit: "fc" }].map(
              (item) => renderOutputItem(item.label, item.value, item.unit),
            ),
            !isNaN(numLuxToFc),
          )}
        ...
        <section id="inverse-square-law-section" className="mb-12">
          ...
          {renderCalcGroup(
            "Illuminance Change with Distance",
            BarChartHorizontalBig,
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
              {renderInputField(
                ...
                "Initial Illuminance",
                initialIlluminance,
                handleInitialIlluminanceChange,
                islUnit,
                "1000",
                Sun,
                "0",
              ...
              {renderSelectField(
                ...
                "Illuminance Unit",
                islUnit,
                (e) => setIslUnit(e.target.value as "lux" | "fc"),
                [
                  { value: "lux", label: "Lux" },
                  { value: "fc", label: "Foot-candles" },
                ],
                Percent,
              )}
            </div>,
            [
              {
                label: `Illuminance at Distance 2`,
                value: formatNumber(illuminanceAtD2),
                unit: islUnit,
              },
            ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
          ...
          {renderCalcGroup(
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-6">
              ...
              {renderSelectField(
                ...
                "Distance Unit",
                beamUnit,
                (e) => setBeamUnit(e.target.value as "meters" | "feet"),
                [
            ...
            [
              {
                label: "Beam Diameter",
                value: formatNumber(calculatedBeamDiameter),
                unit: beamUnit,
              },
            ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
        ...
        <section id="cct-conversion-section" className="mb-12">
          ...
          {renderCalcGroup(
            ...
            [{ label: "MIRED Value", value: formatNumber(calculatedMired, 0), unit: "MIRED" }].map(
              (item) => renderOutputItem(item.label, item.value, item.unit),
            ),
            ...
            [
              {
                ...
                value: formatNumber(calculatedKelvinFromMired, 0),
                unit: "K",
              },
            ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
        ...
        <section id="candela-lumens-section" className="mb-12">
          ...
          {renderCalcGroup(
            "Calculate Lumens from Candela & Beam Angle",
            Sunrise,
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              {renderInputField(
                "candelaValue",
            ...
            [
              {
                label: "Luminous Flux",
                value: formatNumber(calculatedLumensFromCandela),
                unit: "lm",
              },
            ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
        ...
        <section id="throw-distance-section" className="mb-12">
          ...
          {renderCalcGroup(
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-6">
              ...
              {renderInputField(
                "minLuxTarget",
                "Desired Minimum Illuminance (lux)",
                minLuxTarget,
                handleMinLuxTargetChange,
                "lux",
                "200",
                Sun,
                "0",
              ...
              {renderSelectField(
                ...
                "Distance Unit",
                throwDistanceUnit,
                (e) => setThrowDistanceUnit(e.target.value as "meters" | "feet"),
                [
                  { value: "meters", label: "Meters" },
                  { value: "feet", label: "Feet" },
                ],
            ...
            [
              {
                ...
                value: formatNumber(calculatedThrowDistance),
                unit: throwDistanceUnit,
              },
            ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
          ...
          {renderCalcGroup(
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-6">
              ...
              {renderInputField(
                ...
                "Total Area",
                totalAreaSq,
                handleTotalAreaSqChange,
                energyDensityUnit === "m2" ? "m²" : "ft²",
                "100",
                Maximize,
                "0",
              )}
              {renderSelectField(
                ...
                "Area Unit",
                energyDensityUnit,
                (e) => setEnergyDensityUnit(e.target.value as "m2" | "ft2"),
                [
            ...
            [
              {
                ...
                value: formatNumber(calculatedEnergyDensity),
                unit: `W/${energyDensityUnit}`,
              },
            ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
          ...
          {renderCalcGroup(
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
              ...
              {renderSelectField(
                ...
                "Gobo Diameter Unit",
                goboUnits,
                (e) => setGoboUnits(e.target.value as "mm" | "inches"),
                [
              ...
              {renderInputField(
                ...
                "Projection Distance",
                projectionDistance,
                handleProjectionDistanceChange,
                "meters",
                "5",
                Ruler,
                "0",
              ...
              {renderSelectField(
                "projectionSizeUnit",
                "Output Projection Size Unit",
                projectionSizeUnit,
                (e) => setProjectionSizeUnit(e.target.value as "meters" | "feet"),
                [
                  { value: "meters", label: "Meters" },
                  { value: "feet", label: "Feet" },
                ],
            ...
            [
              {
                label: "Projected Image Diameter",
                value: formatNumber(calculatedGoboProjectionSize),
                unit: projectionSizeUnit,
              },
            ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
        ...
        <section id="dmx-offset-section" className="mb-12">
          ...
          {renderCalcGroup(
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              ...
              {renderInputField(
                "dmxChannelOffset",
                "Channel Offset (0-indexed)",
                dmxChannelOffset,
                handleDmxChannelOffsetChange,
                "",
                "0",
                Sigma,
                "0",
            ...
            [
              {
                label: "Absolute DMX Address",
                value: formatNumber(absoluteDmxAddress, 0),
                unit: "",
              },
            ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
        ...
        <section id="power-load-section" className="mb-12">
          ...
          {renderCalcGroup(
            ...
            [
              ...
              {
                label: "Total Current (120V)",
                value: formatNumber(totalAmps120V, 2),
                unit: "Amps",
              },
              {
                label: "Total Current (230V)",
                value: formatNumber(totalAmps230V, 2),
                unit: "Amps",
              },
            ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
            !isNaN(numPowerPerFixture) && !isNaN(numNumberOfFixtures),
