...
const AudioCategoryPage: React.FC = () => {
  ...
  const speedOfSoundMsFt = useMemo(
    () => (speedOfSoundFtMs !== 0 ? 1 / speedOfSoundFtMs : 0),
    [speedOfSoundFtMs],
  );
  const speedOfSoundMMs = useMemo(() => speedOfSoundMPS / 1000, [speedOfSoundMPS]);
  const speedOfSoundMsM = useMemo(
    () => (speedOfSoundMMs !== 0 ? 1 / speedOfSoundMMs : 0),
    [speedOfSoundMMs],
  );
  ...
  const periodMs = useMemo(
    () => (numericFrequencyHz > 0 ? 1000 / numericFrequencyHz : 0),
    [numericFrequencyHz],
  );
  const wavelengthM = useMemo(
    () =>
      numericFrequencyHz > 0 && speedOfSoundMPS > 0 ? speedOfSoundMPS / numericFrequencyHz : 0,
    [numericFrequencyHz, speedOfSoundMPS],
  );
  ...
  const frequencyFromPeriodHz = useMemo(
    () => (numericPeriodInputMs > 0 ? 1000 / numericPeriodInputMs : 0),
    [numericPeriodInputMs],
  );
  const wavelengthFromPeriodM = useMemo(
    () =>
      frequencyFromPeriodHz > 0 && speedOfSoundMPS > 0
        ? speedOfSoundMPS / frequencyFromPeriodHz
        : 0,
    [frequencyFromPeriodHz, speedOfSoundMPS],
  );
  ...
  const numericSamplesInput = useMemo(() => {
    if (typeof numberOfSamplesInput === "number" && numberOfSamplesInput > 0)
      return numberOfSamplesInput;
    return 0;
  ...
  const periodFromSamplesMs = useMemo(
    () => (frequencyFromSamplesHz > 0 ? 1000 / frequencyFromSamplesHz : 0),
    [frequencyFromSamplesHz],
  );
  const wavelengthFromSamplesM = useMemo(
    () =>
      frequencyFromSamplesHz > 0 && speedOfSoundMPS > 0
        ? speedOfSoundMPS / frequencyFromSamplesHz
        : 0,
    [frequencyFromSamplesHz, speedOfSoundMPS],
  );
  ...
  const numericPhaseFrequencyForDelayCalc = useMemo(() => {
    if (typeof phaseFrequencyInput === "number" && phaseFrequencyInput > 0)
      return phaseFrequencyInput;
    return 0;
  }, [phaseFrequencyInput]);
  ...
  const numOhmsVoltage = useMemo(
    () => (typeof ohmsVoltage === "number" ? ohmsVoltage : NaN),
    [ohmsVoltage],
  );
  const numOhmsResistance = useMemo(
    () => (typeof ohmsResistance === "number" && ohmsResistance > 0 ? ohmsResistance : NaN),
    [ohmsResistance],
  );
  const ohmsCurrent = useMemo(
    () =>
      !isNaN(numOhmsVoltage) && !isNaN(numOhmsResistance)
        ? numOhmsVoltage / numOhmsResistance
        : NaN,
    [numOhmsVoltage, numOhmsResistance],
  );
  const ohmsPower = useMemo(
    () => (!isNaN(numOhmsVoltage) && !isNaN(ohmsCurrent) ? numOhmsVoltage * ohmsCurrent : NaN),
    [numOhmsVoltage, ohmsCurrent],
  );

  const numDbPowerP1 = useMemo(
    () => (typeof dbPowerP1 === "number" && dbPowerP1 > 0 ? dbPowerP1 : NaN),
    [dbPowerP1],
  );
  const numDbPowerP0 = useMemo(
    () => (typeof dbPowerP0 === "number" && dbPowerP0 > 0 ? dbPowerP0 : NaN),
    [dbPowerP0],
  );
  const dbPowerRatio = useMemo(
    () =>
      !isNaN(numDbPowerP1) && !isNaN(numDbPowerP0)
        ? 10 * Math.log10(numDbPowerP1 / numDbPowerP0)
        : NaN,
    [numDbPowerP1, numDbPowerP0],
  );

  const numDbVoltageV1 = useMemo(
    () => (typeof dbVoltageV1 === "number" && dbVoltageV1 > 0 ? dbVoltageV1 : NaN),
    [dbVoltageV1],
  );
  const numDbVoltageV0 = useMemo(
    () => (typeof dbVoltageV0 === "number" && dbVoltageV0 > 0 ? dbVoltageV0 : NaN),
    [dbVoltageV0],
  );
  const dbVoltageRatio = useMemo(
    () =>
      !isNaN(numDbVoltageV1) && !isNaN(numDbVoltageV0)
        ? 20 * Math.log10(numDbVoltageV1 / numDbVoltageV0)
        : NaN,
    [numDbVoltageV1, numDbVoltageV0],
  );

  const numdBuInput = useMemo(() => (typeof dBuInput === "number" ? dBuInput : NaN), [dBuInput]);
  const voltsFromDBu = useMemo(
    () => (!isNaN(numdBuInput) ? 0.775 * Math.pow(10, numdBuInput / 20) : NaN),
    [numdBuInput],
  );
  const numVoltsFordBuInput = useMemo(
    () => (typeof voltsFordBuInput === "number" && voltsFordBuInput > 0 ? voltsFordBuInput : NaN),
    [voltsFordBuInput],
  );
  const dBuFromVolts = useMemo(
    () => (!isNaN(numVoltsFordBuInput) ? 20 * Math.log10(numVoltsFordBuInput / 0.775) : NaN),
    [numVoltsFordBuInput],
  );

  const numdBvInput = useMemo(() => (typeof dBvInput === "number" ? dBvInput : NaN), [dBvInput]);
  const voltsFromDBv = useMemo(
    () => (!isNaN(numdBvInput) ? Math.pow(10, numdBvInput / 20) : NaN),
    [numdBvInput],
  );
  const numVoltsFordBvInput = useMemo(
    () => (typeof voltsFordBvInput === "number" && voltsFordBvInput > 0 ? voltsFordBvInput : NaN),
    [voltsFordBvInput],
  );
  const dBvFromVolts = useMemo(
    () => (!isNaN(numVoltsFordBvInput) ? 20 * Math.log10(numVoltsFordBvInput) : NaN),
    [numVoltsFordBvInput],
  );

  // New dBFS calculations
  const numDbfsValue = useMemo(
    () => (typeof dbfsValue === "number" ? dbfsValue : NaN),
    [dbfsValue],
  );
  const dbfsPercentage = useMemo(
    () => (!isNaN(numDbfsValue) ? Math.pow(10, numDbfsValue / 20) * 100 : NaN),
    [numDbfsValue],
  );
  const dbfsLinear = useMemo(
    () => (!isNaN(numDbfsValue) ? Math.pow(10, numDbfsValue / 20) : NaN),
    [numDbfsValue],
  );

  // New SPL Combining calculations
  const numSplSingleSource = useMemo(
    () => (typeof splSingleSource === "number" ? splSingleSource : NaN),
    [splSingleSource],
  );
  const numNumberOfSources = useMemo(
    () => (typeof numberOfSources === "number" && numberOfSources > 0 ? numberOfSources : NaN),
    [numberOfSources],
  );
  const combinedSpl = useMemo(
    () =>
      !isNaN(numSplSingleSource) && !isNaN(numNumberOfSources)
        ? numSplSingleSource + 10 * Math.log10(numNumberOfSources)
        : NaN,
    [numSplSingleSource, numNumberOfSources],
  );

  // New SPL Power Change calculations
  const numSplInitialPower = useMemo(
    () => (typeof splInitialPower === "number" ? splInitialPower : NaN),
    [splInitialPower],
  );
  const numPowerRatio = useMemo(
    () => (typeof powerRatio === "number" && powerRatio > 0 ? powerRatio : NaN),
    [powerRatio],
  );
  const splFromPowerChange = useMemo(
    () =>
      !isNaN(numSplInitialPower) && !isNaN(numPowerRatio)
        ? numSplInitialPower + 10 * Math.log10(numPowerRatio)
        : NaN,
    [numSplInitialPower, numPowerRatio],
  );

  const numInvSqSpl1 = useMemo(
    () => (typeof invSqSpl1 === "number" ? invSqSpl1 : NaN),
    [invSqSpl1],
  );
  const numInvSqDist1 = useMemo(
    () => (typeof invSqDist1 === "number" && invSqDist1 > 0 ? invSqDist1 : NaN),
    [invSqDist1],
  );
  const numInvSqDist2 = useMemo(
    () => (typeof invSqDist2 === "number" && invSqDist2 > 0 ? invSqDist2 : NaN),
    [invSqDist2],
  );
  ...
  const numCombPathDiffM = useMemo(
    () => (typeof combPathDiffM === "number" && combPathDiffM > 0 ? combPathDiffM : NaN),
    [combPathDiffM],
  );
  ...
  const numBitDepth = useMemo(
    () =>
      typeof bitDepth === "number" && bitDepth > 0 && Number.isInteger(bitDepth) ? bitDepth : NaN,
    [bitDepth],
  );
  const dynamicRange = useMemo(
    () => (!isNaN(numBitDepth) ? numBitDepth * 6.02 : NaN),
    [numBitDepth],
  );

  const numFileSizeChannels = useMemo(
    () =>
      typeof fileSizeChannels === "number" &&
      fileSizeChannels > 0 &&
      Number.isInteger(fileSizeChannels)
        ? fileSizeChannels
        : NaN,
    [fileSizeChannels],
  );
  ...
  const audioFileSizeMB = useMemo(() => {
    ...
    if (
      !isNaN(numBitDepth) &&
      !isNaN(numFileSizeChannels) &&
      !isNaN(numFileSizeDurationMin) &&
      sr > 0
    ) {
  ...
  const numSpeakerImpedance1 = useMemo(
    () =>
      typeof speakerImpedance1 === "number" && speakerImpedance1 > 0 ? speakerImpedance1 : NaN,
    [speakerImpedance1],
  );
  const numSpeakerImpedance2 = useMemo(
    () =>
      typeof speakerImpedance2 === "number" && speakerImpedance2 > 0 ? speakerImpedance2 : NaN,
    [speakerImpedance2],
  );
  const seriesImpedance = useMemo(
    () =>
      !isNaN(numSpeakerImpedance1) && !isNaN(numSpeakerImpedance2)
  ...
  const numQFactorFc = useMemo(
    () => (typeof qFactorFc === "number" && qFactorFc > 0 ? qFactorFc : NaN),
    [qFactorFc],
  );
  const numQFactorBw = useMemo(
    () => (typeof qFactorBw === "number" && qFactorBw > 0 ? qFactorBw : NaN),
    [qFactorBw],
  );
  const calculatedQ = useMemo(
    () => (!isNaN(numQFactorFc) && !isNaN(numQFactorBw) ? numQFactorFc / numQFactorBw : NaN),
    [numQFactorFc, numQFactorBw],
  );

  const numBwFactorQ = useMemo(
    () => (typeof bwFactorQ === "number" && bwFactorQ > 0 ? bwFactorQ : NaN),
    [bwFactorQ],
  );
  const calculatedBw = useMemo(
    () => (!isNaN(numQFactorFc) && !isNaN(numBwFactorQ) ? numQFactorFc / numBwFactorQ : NaN),
    [numQFactorFc, numBwFactorQ],
  );

  const numRoomDimensionM = useMemo(
    () => (typeof roomDimensionM === "number" && roomDimensionM > 0 ? roomDimensionM : NaN),
    [roomDimensionM],
  );
  ...
  const handleSimpleNumberChange =
    ...
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!isNaN(numValue)) {
        if (!allowZero && numValue === 0 && !allowNegative && value !== "0.") {
          // Allow "0." for float input
  ...
  const handleSamplesInputChange = handleSimpleNumberChange(
    setNumberOfSamplesInput,
    false,
    false,
    false,
  );
  ...
  const handleNumberOfSourcesChange = handleSimpleNumberChange(
    setNumberOfSources,
    false,
    false,
    false,
  ...
  const handleFileSizeChannelsChange = handleSimpleNumberChange(
    setFileSizeChannels,
    false,
    false,
    false,
  ...
  const handleSpeakerImpedance1Change = handleSimpleNumberChange(
    setSpeakerImpedance1,
    false,
    false,
  );
  const handleSpeakerImpedance2Change = handleSimpleNumberChange(
    setSpeakerImpedance2,
    false,
    false,
  );
      ...
      <main className="flex-grow container mx-auto px-4 py-16 sm:py-24">
        ...
        <section id="constants-section" className="mb-12">
          ...
          <div className="bg-gray-850 p-5 rounded-lg shadow-xl border border-indigo-700/30">
            ...
            <div className="mb-8">
              ...
              {isValidTemperature ? (
                <div>
                  <h4 className="text-lg font-medium text-indigo-100 mb-3">
                    ...
                    {[
                      {
                        ...
                        value: formatNumber(speedOfSoundMPS),
                        unit: "m/s",
                      },
                      {
                        label: "Feet per Second",
                        value: formatNumber(speedOfSoundFPS),
                        unit: "ft/s",
                      },
                      {
                        label: "Meters per Millisecond",
                        value: formatNumber(speedOfSoundMMs, 4),
                        unit: "m/ms",
                      },
                      {
                        label: "Feet per Millisecond",
                        value: formatNumber(speedOfSoundFtMs, 4),
                        unit: "ft/ms",
                      },
                      {
                        label: "Milliseconds per Meter",
                        value: formatNumber(speedOfSoundMsM, 4),
                        unit: "ms/m",
                      },
                      {
                        label: "Milliseconds per Foot",
                        value: formatNumber(speedOfSoundMsFt, 4),
                        unit: "ms/ft",
                      },
                ...
                <p className="text-yellow-400 text-center bg-yellow-900/30 p-3 rounded-md">
                  {" "}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="bg-gray-700 border border-gray-600 text-white focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 pr-4 py-2.5 rounded-md shadow-sm text-lg appearance-none"
                >
              ...
              <p className="mt-3 text-sm text-gray-400">
                ...
                <span className="font-semibold text-indigo-300">{selectedSampleRate} kHz</span>{" "}
              </p>
            </div>
          </div>
        </section>
        ...
        <section id="waveform-properties-section" className="mb-12">
          ...
          <div className="bg-gray-850 p-5 rounded-lg shadow-xl border border-indigo-700/30">
            <p className="text-gray-300 mb-6 text-base leading-relaxed">
              {" "}
              Calculate properties of waveforms based on the defined constants.{" "}
            </p>
            {renderCalcGroup(
              ...
              [
                ...
                { label: "1/4 Wavelength", value: formatNumber(quarterWavelengthM, 3), unit: "m" },
                {
                  ...
                  value: formatNumber(samplesPerCycle, 2),
                  unit: "samples",
                },
                {
                  label: "1/4 Wavelength Period",
                  value: formatNumber(quarterWavelengthPeriodMs, 3),
                  unit: "ms",
                },
              ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
              numericFrequencyHz > 0 && isValidTemperature,
              "Please enter a valid positive frequency and a valid temperature.",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              ...
              [
                ...
                {
                  label: "Samples per Cycle",
                  value: formatNumber(samplesFromPeriod, 2),
                  unit: "samples",
                },
              ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
              numericPeriodInputMs > 0 && isValidTemperature,
              "Please enter a valid positive period and a valid temperature.",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              "Samples & Related Values",
              Calculator,
              <div className="mb-6">
                {renderInputField(
                  "numberOfSamplesInput",
              ...
              numericSamplesInput > 0 && isValidTemperature,
              "Please enter a valid positive number of samples and a valid temperature.",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              "Phase Delay Calculator",
              Shuffle,
              ...
              [{ label: "Phase Delay", value: formatNumber(phaseDelayMs, 4), unit: "ms" }].map(
                (item) => renderOutputItem(item.label, item.value, item.unit),
              ),
              isValidPhaseDegrees && isValidPhaseFrequency,
              "Please enter a valid phase (degrees) and a valid positive frequency.",
            )}
          </div>
        </section>
        ...
        <section id="ohms-law-power-section" className="mb-12">
          ...
          <div className="bg-gray-850 p-5 rounded-lg shadow-xl border border-indigo-700/30">
            {renderCalcGroup(
              ...
              !isNaN(numOhmsVoltage) && !isNaN(numOhmsResistance),
              "Please enter valid voltage and positive resistance.",
            )}
          </div>
        </section>
        ...
        <section id="decibels-levels-section" className="mb-12">
          ...
          <div className="bg-gray-850 p-5 rounded-lg shadow-xl border border-indigo-700/30">
            {renderCalcGroup(
              ...
              [{ label: "Decibels", value: formatNumber(dbPowerRatio, 2), unit: "dB" }].map(
                (item) => renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numDbPowerP1) && !isNaN(numDbPowerP0),
              "Please enter valid positive power values.",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              "dB from Voltage/Pressure Ratio",
              BarChartBig,
              <div className="grid md:grid-cols-2 gap-6 mb-6">
                {renderInputField(
                  "dbVoltageV1",
                  "Voltage/Pressure 1 (V1)",
                  dbVoltageV1,
                  handleDbVoltageV1Change,
                  "units",
                  "1",
                  Zap,
                  "0",
              ...
              [{ label: "Decibels", value: formatNumber(dbVoltageRatio, 2), unit: "dB" }].map(
                (item) => renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numDbVoltageV1) && !isNaN(numDbVoltageV0),
              "Please enter valid positive voltage/pressure values.",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              "dBu to Volts",
              ...
              <div className="mb-6">
                {renderInputField(
                  ...
                  "dBu Value",
                  dBuInput,
                  handleDBuInputChange,
                  "dBu",
                  "0",
                  Sigma,
                )}
              </div>,
              [{ label: "Voltage", value: formatNumber(voltsFromDBu, 3), unit: "V" }].map((item) =>
                renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numdBuInput),
              "Please enter a valid dBu value.",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              "Volts to dBu",
              ...
              [{ label: "dBu", value: formatNumber(dBuFromVolts, 2), unit: "dBu" }].map((item) =>
                renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numVoltsFordBuInput),
            ...
            {renderCalcGroup(
              ...
              <div className="mb-6">
                {renderInputField(
                  ...
                  "dBV Value",
                  dBvInput,
                  handleDBvInputChange,
                  "dBV",
                  "0",
                  Sigma,
                )}
              </div>,
              [{ label: "Voltage", value: formatNumber(voltsFromDBv, 3), unit: "V" }].map((item) =>
                renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numdBvInput),
              "Please enter a valid dBV value.",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              "Volts to dBV",
              ...
              [{ label: "dBV", value: formatNumber(dBvFromVolts, 2), unit: "dBV" }].map((item) =>
                renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numVoltsFordBvInput),
              "Please enter a valid positive voltage.",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              ...
              [
                {
                  label: "Percentage of Full Scale",
                  value: formatNumber(dbfsPercentage, 2),
                  unit: "%",
                },
                { label: "Linear Value (0 to 1)", value: formatNumber(dbfsLinear, 4), unit: "" },
              ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
              !isNaN(numDbfsValue),
              "Please enter a valid dBFS value.",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              "Combining Identical Sound Sources (SPL)",
              Users,
              <div className="grid md:grid-cols-2 gap-6 mb-6">
                {renderInputField(
                  "splSingleSource",
              ...
              [{ label: "Combined SPL", value: formatNumber(combinedSpl, 2), unit: "dB" }].map(
                (item) => renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numSplSingleSource) && !isNaN(numNumberOfSources),
              "Please enter valid SPL and number of sources (must be > 0).",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              "SPL Change with Power Ratio",
              TrendingUp,
              <div className="grid md:grid-cols-2 gap-6 mb-6">
                {renderInputField(
                  "splInitialPower",
              ...
              [{ label: "New SPL", value: formatNumber(splFromPowerChange, 2), unit: "dB" }].map(
                (item) => renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numSplInitialPower) && !isNaN(numPowerRatio),
              "Please enter valid initial SPL and a positive power ratio.",
            )}
          </div>
        </section>
        ...
        <section id="sound-propagation-section" className="mb-12">
          ...
          <div className="bg-gray-850 p-5 rounded-lg shadow-xl border border-indigo-700/30">
            {renderCalcGroup(
              ...
              [{ label: "SPL at Distance 2", value: formatNumber(invSqSpl2, 2), unit: "dB" }].map(
                (item) => renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numInvSqSpl1) && !isNaN(numInvSqDist1) && !isNaN(numInvSqDist2),
              "Please enter valid SPL and positive distances.",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              "Comb Filtering (First Null)",
              AudioWaveform,
              <div className="mb-6">
                {renderInputField(
                  "combPathDiffM",
              ...
              [
                {
                  label: "First Null Frequency",
                  value: formatNumber(firstCombNullFreq, 2),
                  unit: "Hz",
                },
              ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
              !isNaN(numCombPathDiffM) && isValidTemperature,
              "Please enter valid path difference and temperature.",
            )}
          </div>
        </section>
        ...
        <section id="digital-audio-section" className="mb-12">
          ...
          <div className="bg-gray-850 p-5 rounded-lg shadow-xl border border-indigo-700/30">
            {renderCalcGroup(
              ...
              [{ label: "Dynamic Range", value: formatNumber(dynamicRange, 2), unit: "dB" }].map(
                (item) => renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numBitDepth),
              "Please enter a valid positive integer for bit depth.",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              ...
              [{ label: "File Size", value: formatNumber(audioFileSizeMB, 2), unit: "MB" }].map(
                (item) => renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numBitDepth) &&
                !isNaN(numFileSizeChannels) &&
                !isNaN(numFileSizeDurationMin) &&
                parseFloat(selectedSampleRate) > 0,
              "Please enter valid bit depth, channels, duration, and select a sample rate.",
            )}
          </div>
        </section>
        ...
        <section id="speaker-system-section" className="mb-12">
          ...
          <div className="bg-gray-850 p-5 rounded-lg shadow-xl border border-indigo-700/30">
            {renderCalcGroup(
              ...
              [
                {
                  label: "Total Series Impedance",
                  value: formatNumber(seriesImpedance, 2),
                  unit: "Ω",
                },
                {
                  label: "Total Parallel Impedance",
                  value: formatNumber(parallelImpedance, 2),
                  unit: "Ω",
                },
              ].map((item) => renderOutputItem(item.label, item.value, item.unit)),
              !isNaN(numSpeakerImpedance1) && !isNaN(numSpeakerImpedance2),
              "Please enter valid positive impedance values for both speakers.",
            )}
          </div>
        </section>
        ...
        <section id="filters-eq-section" className="mb-12">
          ...
          <div className="bg-gray-850 p-5 rounded-lg shadow-xl border border-indigo-700/30">
            {renderCalcGroup(
              ...
              [{ label: "Q Factor", value: formatNumber(calculatedQ, 3), unit: "" }].map((item) =>
                renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numQFactorFc) && !isNaN(numQFactorBw),
              "Please enter valid positive Fc and Bandwidth.",
            )}
            <hr className="border-gray-700 my-8" />
            {renderCalcGroup(
              "Bandwidth from Fc & Q Factor",
              Gauge,
              <div className="grid md:grid-cols-2 gap-6 mb-6">
                {renderInputField(
                  "qFactorFc_bw",
              ...
              [{ label: "Bandwidth", value: formatNumber(calculatedBw, 2), unit: "Hz" }].map(
                (item) => renderOutputItem(item.label, item.value, item.unit),
              ),
              !isNaN(numQFactorFc) && !isNaN(numBwFactorQ),
              "Please enter valid positive Fc and Q Factor.",
            )}
          </div>
        </section>
        ...
        <section id="room-acoustics-section" className="mb-12">
          ...
          <div className="bg-gray-850 p-5 rounded-lg shadow-xl border border-indigo-700/30">
            {renderCalcGroup(
              "Axial Room Modes (First 3)",
              SquareIcon,
              <div className="mb-6">
                {renderInputField(
                  "roomDimensionM",
                  "Room Dimension (L, W, or H)",
                  roomDimensionM,
                  handleRoomDimensionMChange,
                  "m",
                  "5",
                  Ruler,
                  "0",
              ...
              roomModes
                .map((mode, index) => ({
                  label: `Mode ${index + 1}`,
                  value: formatNumber(mode, 2),
                  unit: "Hz",
                }))
                .map((item) => renderOutputItem(item.label, item.value, item.unit)),
              !isNaN(numRoomDimensionM) && isValidTemperature,
              "Please enter a valid positive room dimension and temperature.",
            )}
          </div>
        </section>
      </main>
