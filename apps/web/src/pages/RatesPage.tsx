...
const RateCard: React.FC<RateCardProps> = ({ roleFamily, titles, rate, overtime, notes }) => (
  <div className="bg-gray-800 p-6 rounded-lg shadow-xl mb-8 transform transition-all duration-300 hover:scale-105">
    <h2 className="text-2xl font-semibold text-indigo-400 mb-3">{roleFamily}</h2>
    <p className="text-sm text-gray-400 mb-1">Common Titles:</p>
    <div className="flex flex-wrap gap-2 mb-4">
      {titles.map((title) => (
        <span key={title} className="bg-gray-700 text-indigo-300 px-3 py-1 rounded-full text-xs">
          {title}
        </span>
      ))}
    </div>
    <p className="text-lg text-gray-200 mb-2">
      <span className="font-semibold">Typical Rate (USD):</span> {rate} (for a 10-hour day unless
      noted)
    </p>
...
const ConsiderationItem: React.FC<{
  ...
  const ratesData: RateCardProps[] = [
    {
      ...
      notes: [
        "Rates can be higher for specialized skills, high-demand events, or significant pre-production work.",
      ],
    },
    {
      ...
      titles: [
        "A2 (Audio Assist)",
        "V2 (Video Assist)",
        "L2 (Lighting Tech/Operator)",
        "Camera Operator (General)",
      ],
      rate: "$450 - $700",
      overtime: [
        "1.5x hourly rate after 10 hours (up to 16 hours)",
    ...
    },
    {
      ...
      notes: [
        "Rates highly dependent on union status and local agreements. Some markets may have minimum call lengths (e.g., 4 hours).",
      ],
    },
    {
      ...
      notes: [
        "Often includes kit fee for specialized gear. Broadcast rates can differ significantly and may include different overtime rules.",
      ],
    },
    {
      ...
      notes: [
        "Rates vary widely based on event scale, complexity, team size, and responsibilities. TD roles often command higher rates.",
      ],
    },
    {
      roleFamily: "Rigging",
      ...
      notes: [
        "Extremely safety-critical role. Certification (e.g., ETCP) often required and commands higher rates. Union agreements are common.",
      ],
    },
  ];
