...
const SharedWithMePage: React.FC = () => {
  ...
  const handleClaimSubmit = async (e: FormEvent) => {
    ...
    if (!code) {
      setClaimMessage(
        "Invalid share code or URL format. Please enter a valid share code or the full share URL.",
      );
      setClaimMessageType("error");
    ...
    try {
      ...
      console.log(
        `[SharedWithMePage] Attempting to claim document. User ID: ${user.id}, Shared Link ID: ${verifiedLink.id}`,
      );
      ...
      setClaimMessage(
        error.message || "An unexpected error occurred while trying to claim the document.",
      );
      setClaimMessageType("error");
    } finally {
      setIsClaimingLoading(false);
  ...
  const handleRemoveClaimedDocument = async (claimedShareId: string) => {
    ...
    setClaimedDocuments((prevDocs) =>
      prevDocs.filter((doc) => doc.claimed_share_id !== claimedShareId),
    );
  ...
  const getResourceTypeIcon = (type: ResourceType) => {
    switch (type) {
      ...
      case "production_schedule":
        return <CalendarDays size={20} className="text-yellow-400" />;
      case "run_of_show":
      ...
      <main className="pt-24 md:pt-28 container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-gray-800 shadow-2xl rounded-xl p-6 sm:p-8 mb-10">
            ...
            {claimMessage && (
              <div
                className={`mt-6 p-4 rounded-md ${claimMessageType === "success" ? "bg-green-700/50 border border-green-500" : "bg-red-700/50 border border-red-500"}`}
              >
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p
                      className={`text-sm font-medium ${claimMessageType === "success" ? "text-green-300" : "text-red-300"}`}
                    >
                      {claimMessage}
                    </p>
                  </div>
            ...
            {!isLoadingDocuments && !fetchDocumentsError && claimedDocuments.length === 0 && (
              <div className="text-center py-10">
                ...
                <p className="text-gray-500 text-sm mt-2">
                  Use the form above to claim documents shared via a link or code.
                </p>
              </div>
            )}
            {!isLoadingDocuments && !fetchDocumentsError && claimedDocuments.length > 0 && (
              <ul className="space-y-4">
                {claimedDocuments.map((doc) => (
                  <li
                    key={doc.claimed_share_id}
                    className="bg-gray-700/50 p-4 rounded-lg shadow-md hover:bg-gray-700 transition-colors duration-150"
                  >
                    <div className="flex flex-col sm:flex-row justify-between items-start">
                      <div className="flex items-center mb-3 sm:mb-0">
                        <span className="mr-3 p-2 bg-gray-600 rounded-md">
                          {getResourceTypeIcon(doc.resource_type)}
                        </span>
                        <div>
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-600/50 text-xs text-gray-500 flex justify-between items-center">
                      ...
                      {doc.original_owner_id && (
                        <span className="italic">
                          Shared by: ...{doc.original_owner_id.slice(-6)}
                        </span>
                      )}
                    </div>
                  </li>
