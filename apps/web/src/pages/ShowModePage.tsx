...
const ShowModePage: React.FC = () => {
  ...
  const findNextPlayableItemIndex = useCallback(
    (startIndex: number | null): number | null => {
  ...
  const findPreviousPlayableItemIndex = useCallback(
    (startIndex: number | null): number | null => {
      if (!runOfShow || !runOfShow.items) return null;
  ...
  useEffect(() => {
    if (currentItemIndex !== null && tableContainerRef.current && runOfShow) {
  ...
  const handleSpacebarPress = useCallback(() => {
    ...
    if (isTimerActive) {
      ...
      if (currentItem?.type === "item" && currentItemDuration !== null) {
        const startDuration =
          timeRemaining !== null && timeRemaining > 0 && timeRemaining <= currentItemDuration
            ? timeRemaining
            : currentItemDuration;
    ...
    return (
      <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-center">
        ...
        <p className="flex-grow flex items-center justify-center">
          Run of Show data could not be loaded.
        </p>
        {!isFullscreen && <Footer />}
      </div>
    );
  ...
  const isPlayPauseDisabled = () => {
    ...
    return (
      !runOfShow.items[currentItemIndex] ||
      parseDurationToSeconds(runOfShow.items[currentItemIndex].duration) === null
    );
  };

  return (
    <div
      className={`min-h-screen flex flex-col bg-gray-950 text-white transition-all duration-300 ${isFullscreen ? "p-1 sm:p-2" : "p-0"}`}
    >
      {!isFullscreen && <Header dashboard={false} />}

      <main
        className={`flex-grow flex flex-col ${isFullscreen ? "p-1 sm:p-2 md:p-4" : "p-4 md:p-8 container mx-auto mt-16 md:mt-12"}`}
      >
        {!isFullscreen && (
        ...
        <div
          className={`flex flex-col sm:flex-row justify-between items-center mb-4 md:mb-6 p-2 sm:p-3 bg-gray-900 rounded-lg shadow-md ${isFullscreen ? "sticky top-0 z-50" : ""}`}
        >
          <div className="flex items-center">
          ...
          <div className="flex items-center space-x-1 sm:space-x-2">
            <button
              ...
              disabled={
                firstPlayableItemActualIndex === null ||
                currentItemIndex === firstPlayableItemActualIndex
              }
            >
              <ChevronsUp size={20} />
            </button>
        ...
        <div className="mt-3 sm:mt-4 text-center text-xs text-gray-500 flex items-center justify-center gap-x-4">
          <span>
            Press{" "}
            <kbd className="px-1.5 sm:px-2 py-0.5 sm:py-1 text-xs font-semibold text-gray-200 bg-gray-700 border border-gray-600 rounded-md">
              Space
            </kbd>{" "}
            to advance/control timer.
          </span>
          {(() => {
            if (
              currentItemIndex !== null &&
              runOfShow &&
              runOfShow.items[currentItemIndex]?.type === "item"
            ) {
              const currentItem = runOfShow.items[currentItemIndex];
              ...
              if (isTimerActive && timeRemaining !== null) {
                ...
                return (
                  <span className="font-mono text-sm text-gray-400">
                    Duration: {formatTime(displayTime)}
                  </span>
                );
              }
            } else if (
              ...
              if (firstItemDuration !== null) {
                return (
                  <span className="font-mono text-sm text-gray-400">
                    Next Duration: {formatTime(firstItemDuration)}
                  </span>
                );
              }
            }
