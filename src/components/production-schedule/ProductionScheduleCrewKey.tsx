...
const predefinedColors = [
  ...
  "#D946EF", "#EC4899", "#F43F5E", "#78716C", "#A3A3A3", "#4B5563",
];
...
const ProductionScheduleCrewKey: React.FC<ProductionScheduleCrewKeyProps> = ({
  ...
  const handleToggleColorPicker = (itemId: string, currentColor: string, event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        const targetIsAnotherTriggerButton = (event.target as HTMLElement).closest(
      ...
      <div className="space-y-4">
        {crewKey.map((item, index) => (
          ...
          >
            ...
            {openColorPickerItemId === item.id && portalPosition && createPortal(
              <div
                ref={colorPickerRef}
              ...
              >
                ...
                <div className="mb-2">
                  <label htmlFor={`hex-color-input-${item.id}`} className="block text-xs text-gray-400 mb-1">HEX Code</label>
                  <input
                    type="text"
                ...
                <div className="mb-3">
                   <label htmlFor={`native-color-picker-${item.id}`} className="block text-xs text-gray-400 mb-1">Custom Color</label>
                  <input
                    type="color"
