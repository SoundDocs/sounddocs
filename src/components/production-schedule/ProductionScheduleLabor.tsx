...
const ProductionScheduleLabor: React.FC<ProductionScheduleLaborProps> = ({ laborItems, onUpdateLaborItems }) => {
  ...
  const handleDuplicateDayGroup = (dateKey: string) => {
    const itemsToDuplicate = laborItems.filter(item => (item.date || 'No Date Assigned') === dateKey);
