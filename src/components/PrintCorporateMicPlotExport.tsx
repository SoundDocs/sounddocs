...
const PrintCorporateMicPlotExport = forwardRef<HTMLDivElement, PrintCorporateMicPlotExportProps>(
  ({ micPlot }, ref) => {
    if (!micPlot) {
      return <div ref={ref} className="p-6 bg-white text-black">Mic plot data not available.</div>;
    }
    ...
    const renderField = (IconComponent: React.ElementType, label: string, value?: string | null | boolean, isTime: boolean = false) => {
      ...
      return (
        <div className="flex items-start text-xs mb-2">
          <IconComponent size={14} className="text-gray-700 mr-2 mt-px flex-shrink-0" /> {/* Icon color changed */}
      ...
      <div ref={ref} className="p-8 bg-white text-black min-w-[1024px] font-sans">
        ...
        <div className="flex justify-between items-center mb-8 pb-4 border-b border-gray-300">
          <div className="flex items-center">
            <div className="p-2 bg-gray-200 rounded-md mr-3"> {/* Header icon bg color changed */}
              <Bookmark className="h-8 w-8 text-gray-700" /> {/* Header icon color changed */}
            </div>
            <div>
              <h1 className="text-2xl font-bold text-black">SoundDocs</h1> {/* Header title color changed */}
            ...
            {micPlot.presenters.map((presenter, index) => (
              <div key={presenter.id || index} className="presenter-card-print">
                <div className="flex flex-row gap-4">
                  <div className="flex-shrink-0 w-[130px]"> {/* Adjusted width for larger photo */}
                    {presenter.photo_url ? (
                      <img src={presenter.photo_url} alt={presenter.presenter_name || 'Presenter'} className="presenter-photo-print" />
                    ) : (
                      <div className="photo-placeholder-print">
                        <ImageIcon size={30} className="mb-1 text-gray-400"/> {/* Increased icon size */}
                        No Photo
                      </div>
                    )}
                    <h3 className="text-md font-semibold text-black mt-2 break-words w-full">{presenter.presenter_name || "N/A"}</h3> {/* Presenter name color changed */}
                    <p className="text-xs text-gray-600 break-words w-full">{presenter.session_segment || "N/A"}</p>
                  </div>
        ...
        <div className="mt-10 pt-6 border-t border-gray-300">
          <div className="flex justify-between items-center text-xs">
            <div className="flex items-center text-gray-700"> {/* Footer text color changed */}
