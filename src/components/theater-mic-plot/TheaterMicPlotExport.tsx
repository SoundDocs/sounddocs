import React, { forwardRef } from "react";
    ...
    const TheaterMicPlotExport = forwardRef<HTMLDivElement, TheaterMicPlotExportProps>(
      ({ micPlot }, ref) => {
        ...
        const renderField = (IconComponent: React.ElementType, label: string, value?: string | null, fullWidth: boolean = false) => {
          ...
          return (
            <div className={`flex items-start text-sm mb-3 ${fullWidth ? 'col-span-2' : ''}`}>
              ...
              <div className="flex-grow min-w-0"> 
                ...
                <span className="ml-1.5 text-gray-200 break-words whitespace-pre-wrap">{String(value)}</span>
              </div>
            </div>
          );
        };
        ...
        return (
          <div ref={ref} className="p-10 bg-gradient-to-br from-gray-900 to-slate-800 text-white min-w-[1000px] font-inter">
            <style>
              {`
                  ...
                  <div key={actor.id || index} className="actor-card">
                    <div className="flex flex-col sm:flex-row gap-6">
                      <div className="flex-shrink-0 sm:w-[180px] flex flex-col items-center sm:items-start">
                        {actor.photo_url ? (
                          <img src={actor.photo_url} alt={actor.actor_name || 'Actor'} className="actor-photo" />
                        ) : (
                        ...
                        <h3 className="text-xl font-semibold text-purple-300 mt-3 text-center sm:text-left break-words w-full">{actor.actor_name || "Unnamed Actor"}</h3>
                        {actor.character_names && <p className="text-sm text-gray-400 text-center sm:text-left break-words w-full">{actor.character_names}</p>}
                      </div>
