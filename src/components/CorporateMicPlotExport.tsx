...
const CorporateMicPlotExport = forwardRef<HTMLDivElement, CorporateMicPlotExportProps>(
  ({ micPlot }, ref) => {
    if (!micPlot) {
      return <div ref={ref} className="p-8 bg-gray-900 text-white">Mic plot data not available.</div>;
    }
    ...
    return (
      <div ref={ref} className="p-10 bg-gradient-to-br from-gray-900 to-slate-900 text-white min-w-[1000px] font-inter">
        <style>
          {`
            ...
            {micPlot.presenters.map((presenter, index) => (
              <div key={presenter.id || index} className="presenter-card">
                <div className="flex flex-col sm:flex-row gap-6">
                  <div className="flex-shrink-0 sm:w-[180px] flex flex-col items-center sm:items-start"> {/* Adjusted width for larger photo */}
                    {presenter.photo_url ? (
                      <img src={presenter.photo_url} alt={presenter.presenter_name || 'Presenter'} className="presenter-photo" />
                    ) : (
                      <div className="photo-placeholder">
                        <ImageIcon size={40} className="mb-1 text-gray-500"/> {/* Increased icon size */}
                        No Photo
                      </div>
                    )}
                    <h3 className="text-xl font-semibold text-indigo-300 mt-3 text-center sm:text-left break-words w-full">{presenter.presenter_name || "Unnamed Presenter"}</h3>
                    <p className="text-sm text-gray-400 text-center sm:text-left break-words w-full">{presenter.session_segment || "No Session Info"}</p>
                  </div>
