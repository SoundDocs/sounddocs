          ...
          {type === 'text' ? (
            ...
            >
              {editingLabel && !disabled ? (
                ...
                <span className="text-white font-medium p-1 whitespace-pre-wrap break-words w-full h-full flex items-center justify-center" style={{ fontSize: 'inherit' }}>
                  {label}
                </span>
              )}
            </div>
            ...
            elementVisualStyles && (
              ...
              >
                {customImageUrl ? (
                  <img src={customImageUrl} alt={label || 'Custom image'} className="object-contain w-full h-full" />
                ) : (
                  getIconForType()
                )}
              </div>
