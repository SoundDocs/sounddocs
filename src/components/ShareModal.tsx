...
import { X, Copy, Check, Link as LinkIcon, Loader2, Share2, Trash2, Clock, AlertTriangle, Edit3, Info } from 'lucide-react';
import {
  createShareLink,
  getShareLinks,
  deleteShareLink,
      ...
      <div className="bg-slate-800 rounded-xl p-6 w-full max-w-xl shadow-2xl text-white max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-start justify-between mb-6 flex-shrink-0">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-slate-100">Share "{resourceName || 'Resource'}"</h2>
        ...
        <div className="flex-grow overflow-y-auto custom-scrollbar pr-2 -mr-4 space-y-6"> {/* Added -mr-4 for scrollbar, space-y-6 for spacing between sections */}
          {error && (
          ...
          <div className="bg-slate-700 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end mb-4">
              <div>
                <label htmlFor="linkType" className="block text-xs font-medium text-slate-300 mb-1">Link Type</label>
                <select
                  id="linkType"
                  value={newLinkType}
            ...
            {newLinkType === 'edit' && (
              <div className="mt-1 mb-4 text-xs text-slate-300 bg-slate-600/70 p-3 rounded-md border border-slate-500/50">
                <div className="flex items-start">
                  ...
                  <div>
                    <p className="font-semibold text-slate-200 mb-1">How recipients use Editable Links:</p>
                    <ol className="list-decimal list-inside space-y-1 text-slate-300/90">
                      <li>Recipient receives the generated link or code.</li>
                      <li>They navigate to their "Shared With Me" page on SoundDocs (account required).</li>
          ...
          <div> {/* Removed flex-grow, overflow-y-auto, pr-1, -mr-2 from here */}
            ...
            <div className="space-y-3"> {/* Added a wrapper for links with space-y-3 */}
                ...
                return (
                  <div key={link.id} className="bg-slate-700 p-4 rounded-lg">
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center space-x-3">
                        <div className={`p-1.5 rounded-md flex-shrink-0 ${isEditLink ? 'bg-amber-600' : 'bg-slate-600'}`}>
                          {isEditLink ? <Edit3 size={20} className="text-amber-300" /> : <LinkIcon size={20} className="text-indigo-400" />}
                        </div>
                        <div>
                    ...
                    <div className="flex items-center space-x-2 bg-slate-900/70 p-2 rounded-md mb-3">
                      ...
                      />
                      <button
                        onClick={() => handleCopyToClipboard(link.id, link.share_code, link.link_type)}
                          ...
                          value={
                            link.expires_at
                              ? expirationOptions.find(opt => {
                                  ...
                                  const now = new Date();
                                  const diffDays = Math.ceil((linkExpDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
