import React, { useEffect, useState, useRef } from "react";
    ...
    const AllTheaterMicPlots = () => {
      ...
      const confirmDelete = async () => {
        ...
        try {
          const { error } = await supabase.from("theater_mic_plots").delete().eq("id", documentToDelete.id);
          if (error) throw error;
      ...
      if (screenSize === "mobile" || screenSize === "tablet") {
        return <MobileScreenWarning title="Screen Size Too Small" description="Theater Mic Plot management requires a larger screen. Please use a desktop or laptop." />;
      }
        ...
        <div className="min-h-screen bg-gray-900 flex flex-col">
          ...
          <main className="flex-grow container mx-auto px-4 py-12 mt-12">
            <div className="flex items-center justify-between mb-8">
              ...
              <button onClick={() => navigate("/theater-mic-plot/new", { state: { from: "/all-theater-mic-plots" } })} className="inline-flex items-center bg-purple-600 hover:bg-purple-700 text-white px-5 py-2 rounded-md font-medium transition-all duration-200">
              ...
              <div className="p-6 border-b border-gray-700">
                <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
                  <div className="relative flex-grow">
                    ...
                    <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-gray-700 text-white w-full pl-10 pr-3 py-2 rounded-md border border-gray-600 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder="Search by name..." />
                  </div>
                    ...
                    {filteredMicPlots.length > 0 ? (
                      filteredMicPlots.map((plot) => (
                        <tr key={plot.id} className="hover:bg-gray-750 transition-colors">
                          <td className="py-4 px-6"><div className="flex items-center"><Drama className="h-5 w-5 text-purple-400 mr-3 flex-shrink-0" /><span className="text-white font-medium">{plot.name}</span></div></td>
                      ...
                      <tr><td colSpan={4} className="py-12 text-center text-gray-400">{searchTerm ? "No theater mic plots match your search criteria" : "You haven't created any theater mic plots yet"}</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              {filteredMicPlots.length > 0 && <div className="p-4 border-t border-gray-700 text-gray-400 text-sm">Showing {filteredMicPlots.length} of {micPlots.length} theater mic plots</div>}
            </div>
          </main>
          ...
          {showShareModal && selectedShareMicPlot && (
            <ShareModal 
              ...
              onClose={() => { setShowShareModal(false); setSelectedShareMicPlot(null); }} 
              ...
              <div className="bg-gray-800 rounded-xl p-6 w-full max-w-md shadow-xl">
                <div className="flex items-start">
                  ...
                  <div className="ml-4 text-left">
                    <h3 className="text-lg font-medium text-white" id="modal-title">Delete Theater Mic Plot</h3>
