{
    ...
    import { 
      ArrowLeft, Clapperboard, Maximize, Minimize, Film, HardDrive, Tv, Smartphone, Sigma, Calculator, Ruler, Percent, RectangleHorizontal, Eye, Projector, DatabaseZap, Video, Zap // Added Video and Zap icons
    } from "lucide-react";
    ...
    const VideoCategoryPage: React.FC = () => {
      ...
      const [projKnownValueType, setProjKnownValueType] = useState<"screenWidth" | "throwDistance">("screenWidth");
      ...
      // Aspect Ratio
      const numArWidth = useMemo(() => typeof arWidth === 'number' && arWidth > 0 ? arWidth : NaN, [arWidth]);
      const numArHeight = useMemo(() => typeof arHeight === 'number' && arHeight > 0 ? arHeight : NaN, [arHeight]);
      ...
      const numArKnownDim = useMemo(() => typeof arKnownDim === 'number' && arKnownDim > 0 ? arKnownDim : NaN, [arKnownDim]);
      const [ratioX, ratioY] = useMemo(() => {
        const parts = arSelectedRatio.split(':');
        return [parseFloat(parts[0]), parseFloat(parts[1])];
      }, [arSelectedRatio]);
      const calculatedDimension = useMemo(() => {
        if (isNaN(numArKnownDim) || isNaN(ratioX) || isNaN(ratioY) || ratioX <= 0 || ratioY <= 0) return NaN;
      ...
      // Resolution to Megapixels
      const numResWidthPx = useMemo(() => typeof resWidthPx === 'number' && resWidthPx > 0 ? resWidthPx : NaN, [resWidthPx]);
      const numResHeightPx = useMemo(() => typeof resHeightPx === 'number' && resHeightPx > 0 ? resHeightPx : NaN, [resHeightPx]);
      const megapixels = useMemo(() => (!isNaN(numResWidthPx) && !isNaN(numResHeightPx)) ? (numResWidthPx * numResHeightPx) / 1000000 : NaN, [numResWidthPx, numResHeightPx]);
      ...
      const numFileSizeBitrateMbps = useMemo(() => typeof fileSizeBitrateMbps === 'number' && fileSizeBitrateMbps > 0 ? fileSizeBitrateMbps : NaN, [fileSizeBitrateMbps]);
      ...
      // Diagonal Screen Size
      const numDiagWidth = useMemo(() => typeof diagWidth === 'number' && diagWidth > 0 ? diagWidth : NaN, [diagWidth]);
      const numDiagHeight = useMemo(() => typeof diagHeight === 'number' && diagHeight > 0 ? diagHeight : NaN, [diagHeight]);
      const screenDiagonal = useMemo(() => (!isNaN(numDiagWidth) && !isNaN(numDiagHeight)) ? Math.sqrt(Math.pow(numDiagWidth, 2) + Math.pow(numDiagHeight, 2)) : NaN, [numDiagWidth, numDiagHeight]);
      const screenArea = useMemo(() => (!isNaN(numDiagWidth) && !isNaN(numDiagHeight)) ? numDiagWidth * numDiagHeight : NaN, [numDiagWidth, numDiagHeight]);

      // Pixels Per Inch (PPI)
      const numPpiResWidth = useMemo(() => typeof ppiResolutionWidth === 'number' && ppiResolutionWidth > 0 ? ppiResolutionWidth : NaN, [ppiResolutionWidth]);
      const numPpiResHeight = useMemo(() => typeof ppiResolutionHeight === 'number' && ppiResolutionHeight > 0 ? ppiResolutionHeight : NaN, [ppiResolutionHeight]);
      const numPpiDiagInches = useMemo(() => typeof ppiDiagonalInches === 'number' && ppiDiagonalInches > 0 ? ppiDiagonalInches : NaN, [ppiDiagonalInches]);
      ...
      // Optimal Viewing Distance
      const numOvdScreenDiagonal = useMemo(() => typeof ovdScreenDiagonal === 'number' && ovdScreenDiagonal > 0 ? ovdScreenDiagonal : NaN, [ovdScreenDiagonal]);
      const { ovdMinDistance, ovdMaxDistance } = useMemo(() => {
        if (isNaN(numOvdScreenDiagonal)) return { ovdMinDistance: NaN, ovdMaxDistance: NaN };
        let minFactor = 1.5, maxFactor = 2.5; // Default for 1080p
        if (ovdResolutionType === "1440p") { minFactor = 1.2; maxFactor = 2.0; }
        else if (ovdResolutionType === "4K") { minFactor = 1.0; maxFactor = 1.5; }
        else if (ovdResolutionType === "8K") { minFactor = 0.75; maxFactor = 1.25; }
        return { ovdMinDistance: numOvdScreenDiagonal * minFactor, ovdMaxDistance: numOvdScreenDiagonal * maxFactor };
      }, [numOvdScreenDiagonal, ovdResolutionType]);

      // Projection Calculator
      const numProjKnownValue = useMemo(() => typeof projKnownValue === 'number' && projKnownValue > 0 ? projKnownValue : NaN, [projKnownValue]);
      const numProjThrowRatio = useMemo(() => typeof projThrowRatio === 'number' && projThrowRatio > 0 ? projThrowRatio : NaN, [projThrowRatio]);
      ...
      // Uncompressed Video Data Rate
      const numUdrWidth = useMemo(() => typeof udrWidth === 'number' && udrWidth > 0 ? udrWidth : NaN, [udrWidth]);
      const numUdrHeight = useMemo(() => typeof udrHeight === 'number' && udrHeight > 0 ? udrHeight : NaN, [udrHeight]);
      const numUdrFrameRate = useMemo(() => typeof udrFrameRate === 'number' && udrFrameRate > 0 ? udrFrameRate : NaN, [udrFrameRate]);
      const numUdrBitDepth = useMemo(() => typeof udrBitDepth === 'number' && udrBitDepth > 0 ? udrBitDepth : NaN, [udrBitDepth]);
      const numUdrColorComponents = useMemo(() => typeof udrColorComponents === 'number' && udrColorComponents > 0 ? udrColorComponents : NaN, [udrColorComponents]);
      const uncompressedDataRateMbps = useMemo(() => {
        if (isNaN(numUdrWidth) || isNaN(numUdrHeight) || isNaN(numUdrFrameRate) || isNaN(numUdrBitDepth) || isNaN(numUdrColorComponents)) return NaN;
        return (numUdrWidth * numUdrHeight * numUdrFrameRate * numUdrBitDepth * numUdrColorComponents) / (1024 * 1024);
      }, [numUdrWidth, numUdrHeight, numUdrFrameRate, numUdrBitDepth, numUdrColorComponents]);
      const uncompressedDataRateMBps = useMemo(() => uncompressedDataRateMbps / 8, [uncompressedDataRateMbps]);

      // Flicker-Free Shutter Speed
      const numLightSourceFreq = useMemo(() => typeof lightSourceFreq === 'number' && lightSourceFreq > 0 ? lightSourceFreq : NaN, [lightSourceFreq]);
      const numPwmDimmingFreq = useMemo(() => typeof pwmDimmingFreq === 'number' && pwmDimmingFreq > 0 ? pwmDimmingFreq : (typeof pwmDimmingFreq === 'string' && pwmDimmingFreq === "" ? null : NaN), [pwmDimmingFreq]);
      ...
      const flickerFreeShutterSpeeds = useMemo(() => {
        ...
        const commonDenominators = [24, 25, 30, 48, 50, 60, 100, 120, 125, 150, 180, 200, 240, 250, 300, 360, 400, 500, 720, 1000];
        let safeSpeeds: string[] = [];

        for (let i = 1; i <= 5; i++) {
            const idealDenominator = i * targetFreq;
            commonDenominators.forEach(denom => {
                if (Math.abs(denom - idealDenominator) < (idealDenominator * 0.1) || denom % Math.round(targetFreq) === 0) {
        ...
        commonDenominators.filter(d => d > targetFreq * 2).forEach(denom => {
             if (!safeSpeeds.includes(`1/${denom}s`)) {
                safeSpeeds.push(`1/${denom}s`);
            }
        });
      ...
      // --- Input Handlers ---
      const handleSimpleNumberChange = (setter: React.Dispatch<React.SetStateAction<number | string>>, allowZero: boolean = false, allowFloat: boolean = true) => (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        } else if (value.endsWith(".") && allowFloat) { setter(value); }
      };
      ...
      const handleFileSizeDurationMinChange = handleSimpleNumberChange(setFileSizeDurationMin, false, true);
      const handleFileSizeBitrateMbpsChange = handleSimpleNumberChange(setFileSizeBitrateMbps, false, true);
      ...
      const handleUdrColorComponentsChange = handleSimpleNumberChange(setUdrColorComponents, false, false);
        ...
        <div className="bg-gray-700/70 p-4 rounded-md shadow sm:col-span-2 lg:col-span-3">
          ...
          {values.length > 0 ? (
            <ul className="list-disc list-inside mt-1">
              {values.map((val, idx) => (
                <li key={idx} className="text-lg text-white">{val} <span className="text-sm font-normal text-gray-400">{unit}</span></li>
              ))}
            </ul>
          ) : (
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              min={min}
              step={min === "0" || (typeof value === 'string' && value.includes('.')) || (typeof value === 'number' && !Number.isInteger(value)) ? "any" : "1"}
            />
          </div>
        </div>
      );
      ...
      const renderSelectField = (id: string, label: string, value: string | number, onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void, options: {value: string | number, label: string}[], icon?: React.ElementType) => (
        <div>
          <label htmlFor={id} className="block text-sm font-medium text-green-300 mb-2">{label}</label>
          <div className="relative">
            {icon && <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              {React.createElement(icon, { className: "h-5 w-5 text-gray-400", "aria-hidden": true })}
            </div>}
        ...
        <div className="min-h-screen bg-gray-900 text-white flex flex-col">
          ...
          <main className="flex-grow container mx-auto px-4 py-16 sm:py-24">
            ...
            <section id="aspect-ratio-section" className="mb-12">
              ...
              {renderCalcGroup("Calculate Aspect Ratio from Dimensions", RectangleHorizontal,
                (<div className="grid md:grid-cols-2 gap-6 mb-6">
                  {renderInputField("arWidth", "Width", arWidth, handleArWidthChange, "units", "1920", Maximize, "1")}
                  {renderInputField("arHeight", "Height", arHeight, handleArHeightChange, "units", "1080", Maximize, "1")}
                </div>),
                [
                  { label: "Aspect Ratio", value: calculatedAspectRatio, unit: "" },
                ].map(item => renderOutputItem(item.label, item.value, item.unit)),
                !isNaN(numArWidth) && !isNaN(numArHeight)
              )}
              <hr className="border-gray-700 my-8" />
              {renderCalcGroup("Calculate Dimension from Aspect Ratio", RectangleHorizontal,
                (<div className="grid md:grid-cols-3 gap-6 mb-6">
                  {renderInputField("arKnownDim", "Known Dimension", arKnownDim, handleArKnownDimChange, "units", "1920", Ruler, "0")}
                  {renderSelectField("arKnownType", "Known Dimension Type", arKnownType, (e) => setArKnownType(e.target.value as "width" | "height"), [{value: "width", label: "Width"}, {value: "height", label: "Height"}], RectangleHorizontal)}
                  {renderSelectField("arSelectedRatio", "Select Aspect Ratio", arSelectedRatio, (e) => setArSelectedRatio(e.target.value), commonAspectRatios, RectangleHorizontal)}
                </div>),
                [
                  { label: `Calculated ${arKnownType === "width" ? "Height" : "Width"}`, value: formatNumber(calculatedDimension), unit: "units" },
                ].map(item => renderOutputItem(item.label, item.value, item.unit)),
            ...
            <section id="resolution-megapixels-section" className="mb-12">
              ...
              {renderCalcGroup("Resolution to Megapixels", Tv,
                (<div className="grid md:grid-cols-2 gap-6 mb-6">
                  {renderInputField("resWidthPx", "Width", resWidthPx, handleResWidthPxChange, "pixels", "1920", Maximize, "1")}
                  {renderInputField("resHeightPx", "Height", resHeightPx, handleResHeightPxChange, "pixels", "1080", Maximize, "1")}
                </div>),
                [
                  { label: "Total Megapixels", value: formatNumber(megapixels, 2), unit: "MP" },
                ].map(item => renderOutputItem(item.label, item.value, item.unit)),
            ...
            <section id="video-file-size-section" className="mb-12">
              ...
              {renderCalcGroup("Estimate File Size (Compressed)", HardDrive,
                (<div className="grid md:grid-cols-2 gap-6 mb-6">
                  {renderInputField("fileSizeDurationMin", "Duration", fileSizeDurationMin, handleFileSizeDurationMinChange, "minutes", "60", Film, "0")}
                  {renderInputField("fileSizeBitrateMbps", "Bitrate", fileSizeBitrateMbps, handleFileSizeBitrateMbpsChange, "Mbps", "10", Sigma, "0")}
                </div>),
                [
                  { label: "Estimated File Size (MB)", value: formatNumber(videoFileSizeMB, 2), unit: "MB" },
                  { label: "Estimated File Size (GB)", value: formatNumber(videoFileSizeGB, 2), unit: "GB" },
                ].map(item => renderOutputItem(item.label, item.value, item.unit)),
            ...
            <section id="screen-dimensions-section" className="mb-12">
              ...
              {renderCalcGroup("Diagonal Screen Size & Area", Smartphone,
                (<div className="grid md:grid-cols-3 gap-6 mb-6">
                  {renderInputField("diagWidth", "Width", diagWidth, handleDiagWidthChange, diagUnit, "27", Maximize, "0")}
                  {renderInputField("diagHeight", "Height", diagHeight, handleDiagHeightChange, diagUnit, "15.1875", Maximize, "0")}
                  {renderSelectField("diagUnit", "Unit", diagUnit, (e) => setDiagUnit(e.target.value as "inches" | "cm"), [{value: "inches", label: "Inches"}, {value: "cm", label: "Centimeters"}], Ruler)}
                </div>),
                [
              ...
              <hr className="border-gray-700 my-8" />
              {renderCalcGroup("Pixels Per Inch (PPI)", Percent,
                (<div className="grid md:grid-cols-3 gap-6 mb-6">
                  {renderInputField("ppiResolutionWidth", "Resolution Width", ppiResolutionWidth, handlePpiResWidthChange, "pixels", "1920", Maximize, "1")}
                  {renderInputField("ppiResolutionHeight", "Resolution Height", ppiResolutionHeight, handlePpiResHeightChange, "pixels", "1080", Maximize, "1")}
                  {renderInputField("ppiDiagonalInches", "Diagonal Size", ppiDiagonalInches, handlePpiDiagInchesChange, "inches", "27", Smartphone, "0")}
                </div>),
                [
                  { label: "Pixels Per Inch", value: formatNumber(ppiValue, 0), unit: "PPI" },
                ].map(item => renderOutputItem(item.label, item.value, item.unit)),
            ...
            <section id="optimal-viewing-distance-section" className="mb-12">
              ...
              {renderCalcGroup("Calculate Viewing Distance", Eye,
                (<div className="grid md:grid-cols-3 gap-6 mb-6">
                  {renderInputField("ovdScreenDiagonal", "Screen Diagonal", ovdScreenDiagonal, handleOvdScreenDiagonalChange, ovdUnit, "55", Smartphone, "0")}
                  {renderSelectField("ovdResolutionType", "Screen Resolution", ovdResolutionType, (e) => setOvdResolutionType(e.target.value), resolutionTypes, Tv)}
                  {renderSelectField("ovdUnit", "Unit", ovdUnit, (e) => setOvdUnit(e.target.value as "inches" | "feet" | "cm" | "meters"), 
                    [{value: "inches", label: "Inches"}, {value: "feet", label: "Feet"}, {value: "cm", label: "Centimeters"}, {value: "meters", label: "Meters"}], Ruler)}
                </div>),
                [
                  { label: "Min Recommended Distance", value: formatNumber(ovdMinDistance, 1), unit: ovdUnit },
                  { label: "Max Recommended Distance", value: formatNumber(ovdMaxDistance, 1), unit: ovdUnit },
                ].map(item => renderOutputItem(item.label, item.value, item.unit)),
            ...
            <section id="projection-calculator-section" className="mb-12">
              ...
              {renderCalcGroup("Calculate Projection Parameters", Projector,
                (<div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                  {renderSelectField("projKnownValueType", "Calculate For", projKnownValueType, (e) => setProjKnownValueType(e.target.value as "screenWidth" | "throwDistance"), 
                    [{value: "throwDistance", label: "Throw Distance"}, {value: "screenWidth", label: "Screen Width"}], Calculator)}
                  {renderInputField("projKnownValue", projKnownValueType === "screenWidth" ? "Known Screen Width" : "Known Throw Distance", projKnownValue, handleProjKnownValueChange, projUnit, projKnownValueType === "screenWidth" ? "10" : "15", Ruler, "0")}
                  {renderInputField("projThrowRatio", "Throw Ratio (e.g., 1.5 for 1.5:1)", projThrowRatio, handleProjThrowRatioChange, "", "1.5", Sigma, "0")}
                  {renderSelectField("projUnit", "Unit", projUnit, (e) => setProjUnit(e.target.value as "feet" | "meters"), 
                    [{value: "feet", label: "Feet"}, {value: "meters", label: "Meters"}], Ruler)}
                </div>),
                [
                  { label: `Calculated ${projKnownValueType === "screenWidth" ? "Throw Distance" : "Screen Width"}`, value: formatNumber(projCalculatedValue, 2), unit: projUnit },
                ].map(item => renderOutputItem(item.label, item.value, item.unit)),
            ...
            <section id="uncompressed-data-rate-section" className="mb-12">
              ...
              {renderCalcGroup("Estimate Uncompressed Data Rate", DatabaseZap,
                (<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                  {renderInputField("udrWidth", "Resolution Width", udrWidth, handleUdrWidthChange, "pixels", "1920", Maximize, "1")}
                  {renderInputField("udrHeight", "Resolution Height", udrHeight, handleUdrHeightChange, "pixels", "1080", Maximize, "1")}
                  {renderInputField("udrFrameRate", "Frame Rate", udrFrameRate, handleUdrFrameRateChange, "fps", "30", Film, "1")}
                  {renderSelectField("udrBitDepth", "Bit Depth (per component)", udrBitDepth, (e) => setUdrBitDepth(Number(e.target.value)), commonBitDepths, Sigma)}
                  {renderInputField("udrColorComponents", "Color Components (e.g., 3 for RGB)", udrColorComponents, handleUdrColorComponentsChange, "", "3", Sigma, "1")}
                </div>),
                [
                  { label: "Data Rate (Mbps)", value: formatNumber(uncompressedDataRateMbps, 2), unit: "Mbps" },
                  { label: "Data Rate (MB/s)", value: formatNumber(uncompressedDataRateMBps, 2), unit: "MB/s" },
                ].map(item => renderOutputItem(item.label, item.value, item.unit)),
                !isNaN(numUdrWidth) && !isNaN(numUdrHeight) && !isNaN(numUdrFrameRate) && !isNaN(numUdrBitDepth) && !isNaN(numUdrColorComponents)
              )}
            </section>
            ...
            <section id="flicker-free-shutter-section" className="mb-12">
              ...
              {renderCalcGroup("Suggested Shutter Speeds", Video,
                (<div className="grid md:grid-cols-2 gap-6 mb-6">
                  {renderInputField("lightSourceFreq", "Mains/Light Frequency", lightSourceFreq, handleLightSourceFreqChange, "Hz (e.g., 100 or 120 for AC)", "120", Zap, "0")}
                  {renderInputField("pwmDimmingFreq", "LED PWM Frequency (Optional)", pwmDimmingFreq, handlePwmDimmingFreqChange, "Hz (if known)", "2000", Percent, "0")}
                </div>),
                renderOutputList("Suggested Flicker-Safe Shutter Speeds", flickerFreeShutterSpeeds, ""),
                (!isNaN(numLightSourceFreq) || (numPwmDimmingFreq !== null && !isNaN(numPwmDimmingFreq))) && flickerFreeShutterSpeeds.length > 0,
