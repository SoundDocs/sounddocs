import React from "react";
    ...
    import { ChevronLeft, Zap, AudioWaveform, Video, Lightbulb, Network, Speaker, Tv, Smartphone, Usb, PowerSquare } from "lucide-react";
    ...
    const connectorsData: ConnectorInfo[] = [
      {
        ...
        notes: "For AES/EBU (digital audio), Pin 2 is Data + and Pin 3 is Data -. Wire colors inside cables are not universally standardized beyond the pin functions."
      },
      {
        id: "trs-stereo",
      ...
      },
      {
        ...
        commonUses: ["Electric guitars", "Keyboards (mono output)", "Effects pedals", "Unbalanced line-level connections"],
        pinout: [
      ...
      },
      {
        ...
        category: "Audio",
        description: "Tip-Ring-Ring-Sleeve connector, commonly for stereo audio output and microphone input.",
        ...
        notes: "OMTP is an older, less common standard with Mic and Ground swapped. CTIA (Cellular Telecommunications Industry Association) / AHJ (American Headset Jack) is the modern standard."
      },
      {
        id: "rca",
        ...
        commonUses: ["Consumer audio/video equipment", "DJ turntables (phono)", "S/PDIF digital audio", "Composite video", "Component video (YPbPr)"],
        pinout: [
          { pin: "Center Pin", signal: "Signal" },
          { pin: "Outer Shield", signal: "Ground" },
        ],
        icon: <Tv className="h-6 w-6 text-orange-300" />,
        notes: "Color coding is common: White (Left Audio), Red (Right Audio), Yellow (Composite Video). For component video: Green (Y), Blue (Pb/Cb), Red (Pr/Cr)."
      },
      {
        id: "dmx5",
        name: "DMX512 (5-Pin XLR)",
        category: "Lighting",
        description: "Standard for lighting control, allowing for a primary and optional secondary data link.",
        ...
        notes: "Pins 4 and 5 are often unused. Wire colors often follow twisted pair conventions if using Cat5-style cable (e.g., Orange/White-Orange for Data 1, Green/White-Green for Data 2)."
      },
      {
        id: "dmx3",
        ...
        icon: <Lightbulb className="h-6 w-6 text-yellow-400" />,
        notes: "While common, 5-pin XLR is the official standard to avoid confusion with audio XLR and allow for the secondary data link."
      },
      {
        id: "sdi",
        ...
        notes: "Requires 75-ohm coaxial cable and BNC connectors. No specific wire colors beyond cable jacket."
      },
      {
        id: "bnc-wordclock",
        name: "BNC (Word Clock / Video Sync)",
        category: "AV/Data",
        description: "BNC connector used for synchronization signals like Word Clock (audio) or Black Burst/Tri-Level Sync (video).",
        ...
        icon: <Video className="h-6 w-6 text-red-400" />,
        notes: "Requires 75-ohm coaxial cable for video sync and word clock. Word clock can sometimes use 50-ohm cables/termination depending on equipment."
      },
      {
        id: "nl2",
        name: "Speakon NL2",
      ...
      },
      {
        ...
        name: "Speakon NL4",
        category: "Audio",
        description: "4-pole connector for loudspeaker connections, supporting up to two channels or bi-amping.",
        ...
        icon: <AudioWaveform className="h-6 w-6 text-sky-400" />,
        notes: "Can be wired for single channel (using 1+/1-) or two channels/bi-amping. Colors can vary; always check documentation or test."
      },
      {
        id: "nl8",
        name: "Speakon NL8",
        category: "Audio",
        description: "8-pole connector for loudspeaker connections, supporting up to four channels or complex multi-way systems.",
        commonUses: ["Multi-way passive loudspeaker systems", "Amplifier racks"],
        pinout: [
          { pin: "1+", signal: "Channel 1 Positive (+)" }, { pin: "1-", signal: "Channel 1 Negative (-)" },
          { pin: "2+", signal: "Channel 2 Positive (+)" }, { pin: "2-", signal: "Channel 2 Negative (-)" },
          { pin: "3+", signal: "Channel 3 Positive (+)" }, { pin: "3-", signal: "Channel 3 Negative (-)" },
          { pin: "4+", signal: "Channel 4 Positive (+)" }, { pin: "4-", signal: "Channel 4 Negative (-)" },
        ],
        icon: <AudioWaveform className="h-6 w-6 text-sky-400" />,
        notes: "Wire colors are not strictly standardized for all 8 poles but often follow a logical progression from NL4 colors or use numbered/striped wires."
      },
      {
        id: "rj45",
        name: "Ethernet (RJ45 - T568B)",
        category: "Data",
        description: "Standard connector for networking, also used for digital audio (AoIP) and lighting control.",
        commonUses: ["Computer networking", "Audio over IP (e.g., Dante)", "Art-Net/sACN lighting control"],
        pinout: [
        ...
        notes: "T568A is an alternative wiring standard (swaps Orange and Green pairs). For PoE (Power over Ethernet), pins 4,5,7,8 may carry power."
      },
      {
        id: "hdmi-a",
        name: "HDMI (Type A)",
        ...
        notes: "This is a simplified overview. HDMI has 19 pins with complex signaling. Wire colors within the cable are not standardized for end-users."
      },
      {
        id: "usb-a",
        name: "USB-A",
        ...
        commonUses: ["Connecting peripherals to computers (keyboards, mice, flash drives)", "Charging devices"],
        pinout: [
      ...
      },
      {
        ...
        commonUses: ["Modern smartphones, laptops, tablets", "Docking stations", "External storage", "DisplayPort Alternate Mode"],
        pinout: [
        ...
        icon: <Usb className="h-6 w-6 text-blue-400" />,
        notes: "USB-C is highly complex with 24 pins. This is a simplified overview. Supports USB Power Delivery (PD) and alternate modes like DisplayPort/Thunderbolt."
      },
      {
        id: "edison",
      ...
      },
      {
        ...
        pinout: [
          ...
          { pin: "Earth (E)", signal: "Protective Earth / Ground", color: "Green/Yellow (IEC) / Green (US)" },
        ],
      ...
      },
      {
        ...
        pinout: [
          ...
          { pin: "Y (Brass)", signal: "Hot 2 (Line 2)", color: "Red (for 208V) / White (for 240V from split-phase)" },
          { pin: "G (Green)", signal: "Ground", color: "Green" },
        ],
        icon: <Zap className="h-6 w-6 text-orange-400" />,
        notes: "Color for Hot 2 can vary based on the source (e.g., Red for 208V three-phase derived, White if it's L2 of a 120/240V split-phase system)."
      },
      {
        id: "l2130",
      ...
      },
      {
        ...
        name: "Socapex (19-Pin)",
        category: "Power",
        description: "Multi-pin connector for distributing multiple lighting circuits (typically 6 circuits of 20A).",
        ...
        icon: <Zap className="h-6 w-6 text-orange-400" />,
        notes: "Common Hot wire colors: Ckt 1 (Black), Ckt 2 (Red), Ckt 3 (Blue), Ckt 4 (Brown or second Black), Ckt 5 (Orange or second Red), Ckt 6 (Yellow or second Blue). Neutrals are typically White (sometimes with color trace), Grounds are Green (sometimes with color trace). Always verify wiring."
      },
      {
        id: "true1",
        name: "powerCON TRUE1 TOP",
        category: "Power",
        description: "Locking, true mains connector with breaking capacity (can be connected/disconnected under load). IP65 rated.",
        ...
        icon: <Zap className="h-6 w-6 text-orange-400" />,
        notes: "TOP (Total Outdoor Protection) version is IP65 rated and backwards compatible with older TRUE1."
      },
      {
        id: "powercon",
        name: "powerCON (Blue/Grey - NAC3MPA/B, NAC3FCA/B)",
        category: "Power",
        description: "Locking AC power connector. Blue (NAC3FCA/MPA) for power-in, Grey (NAC3FCB/MPB) for power-out. Not for breaking capacity.",
        ...
        icon: <Zap className="h-6 w-6 text-orange-400" />,
        notes: "Blue (power-in) and Grey (power-out) connectors are keyed differently and are not intermatable. Always de-energize before connecting/disconnecting."
      },
    ];
      ...
      return (
        <div id={connector.id} className="bg-gray-800 p-6 rounded-lg shadow-lg scroll-mt-24">
          <div className="flex items-center mb-4">
            {connector.icon && <div className="mr-3 p-2 bg-gray-700 rounded-full">{connector.icon}</div>}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm text-left text-gray-400">
              <thead className="text-xs text-gray-300 uppercase bg-gray-700">
                <tr>
                  <th scope="col" className="px-4 py-2">Pin</th>
                  <th scope="col" className="px-4 py-2">Signal/Function</th>
                  {connector.pinout.some(p => p.color) && <th scope="col" className="px-4 py-2">Typical Wire Color</th>}
                </tr>
              </thead>
              <tbody>
                {connector.pinout.map((p, index) => (
                  <tr key={index} className="bg-gray-800 border-b border-gray-700 hover:bg-gray-750">
                    ...
                    {connector.pinout.some(item => item.color) && <td className="px-4 py-2">{p.color || "-"}</td>}
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {connector.notes && (
            <div className="mt-4 p-3 bg-gray-750 rounded-md">
              <p className="text-sm text-gray-300"><strong className="text-sky-400">Note:</strong> {connector.notes}</p>
            </div>
          )}
        </div>
      );
    ...
    const categoryOrder: ConnectorInfo["category"][] = ["Audio", "Video", "Lighting", "Power", "Data", "AV/Data"];
        ...
        <div className="min-h-screen bg-gray-900 text-white flex flex-col">
          ...
          <main className="flex-grow container mx-auto px-4 py-16">
            ...
            <p className="text-lg text-gray-400 text-center mb-12 max-w-2xl mx-auto">
              A quick reference for pin assignments on frequently used audio, video, lighting, power, and data connectors. Wire colors are typical and can vary. Always verify with a tester or manufacturer's documentation.
            </p>

            <div className="bg-gray-800 py-4 mb-8 rounded-lg shadow-md">
              <h3 className="text-lg font-semibold text-sky-300 text-center mb-3">Quick Jump to Connector:</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 px-4">
                  {sortedConnectors.map((connector) => ( // Use sortedConnectors for Quick Jump as well
                      <a 
                          href={`#${connector.id}`} 
                          key={connector.id}
            ...
            <div className="space-y-8">
              {sortedConnectors.map((connector) => ( // Use sortedConnectors for the main list
