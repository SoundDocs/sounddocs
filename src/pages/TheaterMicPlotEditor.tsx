import React, { useState, useEffect } from 'react';
    ...
    import { Loader, ArrowLeft, Save, PlusCircle, AlertCircle, Users, Drama, Share2 } from 'lucide-react';
    import { getSharedResource, updateSharedResource, verifyShareLink, SharedLink } from '../lib/shareUtils'; // Import share utilities
    ...
    const TheaterMicPlotEditor: React.FC = () => {
      ...
      useEffect(() => {
        const initializeEditor = async () => {
          ...
          if (shareCode) { // Handling shared edit link
          ...
          } else { // Handling owned document or new document
            ...
            if (routeId === 'new') {
              ...
              try {
                ...
                if (data) {
                  ...
                  navigate('/audio', { replace: true, state: { error: 'Theater mic plot not found or access denied.' } });
                }
              } catch (error) {
                console.error('Error fetching theater mic plot:', error);
                navigate('/audio', { replace: true, state: { error: 'Failed to load theater mic plot.' } });
              } finally {
                setLoading(false);
              }
            }
      ...
      const handleSave = async () => {
        ...
        try {
          if (isSharedEdit && shareCode) {
            const updatedResource = await updateSharedResource(shareCode, 'theater_mic_plot', dataToSave);
            setMicPlot(updatedResource as TheaterMicPlot);
          } else if (user) { // Standard save for owned document
            if (routeId === 'new') {
              const { data, error } = await supabase.from('theater_mic_plots').insert(dataToSave).select().single();
              ...
              const { data, error } = await supabase.from('theater_mic_plots').update(dataToSave).eq('id', micPlot.id).eq('user_id', user.id).select().single();
              if (error) throw error;
      ...
      if (loading || !micPlot) {
        return <div className="min-h-screen bg-gray-900 flex items-center justify-center"><Loader className="h-12 w-12 text-purple-500 animate-spin" /></div>;
      }
      ...
      if (!isSharedEdit && !user) {
         return <div className="min-h-screen bg-gray-900 flex items-center justify-center"><p className="text-red-500">Authentication required.</p></div>;
      }
      ...
      return (
        <div className="min-h-screen bg-gray-900 flex flex-col">
          ...
          <main className={`flex-grow container mx-auto px-4 py-6 md:py-12 ${isSharedEdit ? 'mt-20 md:mt-20' : 'mt-16 md:mt-12'}`}>
                ...
                <div className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-4 mb-6 flex items-start">
                    ...
                    <div>
                        <p className="text-purple-300 text-sm">
                            You are editing a shared Theater Mic Plot. Changes will be saved to the shared version.
                        </p>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 md:mb-8 gap-4">
              <div className="flex items-center flex-grow min-w-0">
                <button onClick={handleBackNavigation} className="mr-2 md:mr-4 flex items-center text-gray-400 hover:text-white transition-colors flex-shrink-0">
                  <ArrowLeft className="h-5 w-5" />
                </button>
                <div className="flex-grow min-w-0">
                  <input type="text" value={micPlot.name} onChange={handleNameChange} className="text-xl md:text-2xl font-bold text-white bg-transparent border-none focus:outline-none focus:ring-0 w-full" placeholder="Enter Theater Mic Plot Name"/>
                  <p className="text-xs sm:text-sm text-gray-400 truncate">Last edited: {new Date(micPlot.last_edited).toLocaleString()}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 fixed bottom-4 right-4 z-20 md:static md:z-auto sm:ml-auto flex-shrink-0">
                <button onClick={handleSave} disabled={saving} className="inline-flex items-center bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-md font-medium transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed shadow-lg md:shadow-none">
            ...
            {saveError && <div className="bg-red-400/10 border border-red-400 rounded-lg p-4 mb-4 flex items-start"><AlertCircle className="h-5 w-5 text-red-400 mr-3 mt-0.5 flex-shrink-0" /><p className="text-red-400 text-sm">{saveError}</p></div>}
            {saveSuccess && <div className="bg-green-400/10 border border-green-400 rounded-lg p-4 mb-4 flex items-start"><Save className="h-5 w-5 text-green-400 mr-3 mt-0.5 flex-shrink-0" /><p className="text-green-400 text-sm">Theater mic plot saved successfully!</p></div>}
            ...
            <div className="bg-gray-800 rounded-xl shadow-lg p-4 md:p-6 mb-8">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={handleAddActor} className="inline-flex items-center bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md font-medium transition-colors text-sm">
              ...
              {micPlot.actors.map((entry) => (
                <ActorEntryCard key={entry.id} entry={entry} onUpdate={handleUpdateActor} onDelete={handleDeleteActor} />
              ))}
            </div>
            
            <div className="flex justify-center py-8">
              <button onClick={handleSave} disabled={saving} className="inline-flex items-center bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-md font-medium transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed shadow-lg text-base">
