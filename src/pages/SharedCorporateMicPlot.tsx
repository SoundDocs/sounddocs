          ...
          <div className="corporate-mic-plot-display">
            ...
            {presenters.length > 0 ? (
                  ...
                  <div key={presenter.id || index} className="bg-gray-800 rounded-lg p-6 shadow-md">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center mb-4">
                      ...
                      <div className="flex-grow">
                        <h3 className="text-xl font-semibold text-white mb-1">{presenter.presenter_name || "Unnamed Presenter"}</h3>
                        {presenter.session_segment && <p className="text-indigo-400 text-sm">{presenter.session_segment}</p>}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                      {presenter.mic_type && (
                        <div className="bg-gray-700/50 p-3 rounded-md">
                          <p className="text-gray-400 font-medium flex items-center"><Radio className="h-4 w-4 mr-2 text-indigo-400" />Mic Type</p>
                          <p className="text-white">{presenter.mic_type}</p>
                        </div>
                      )}
                      {presenter.element_channel_number && (
                        <div className="bg-gray-700/50 p-3 rounded-md">
                          <p className="text-gray-400 font-medium flex items-center"><Speaker className="h-4 w-4 mr-2 text-indigo-400" />Element/Channel</p>
                          <p className="text-white">{presenter.element_channel_number}</p>
                        </div>
                      )}
                      {presenter.tx_pack_location && (
                        <div className="bg-gray-700/50 p-3 rounded-md">
                          <p className="text-gray-400 font-medium flex items-center"><Briefcase className="h-4 w-4 mr-2 text-indigo-400" />TX Pack Location</p>
                          <p className="text-white">{presenter.tx_pack_location}</p>
                        </div>
                      )}
                      {presenter.backup_element && (
                        <div className="bg-gray-700/50 p-3 rounded-md">
                          <p className="text-gray-400 font-medium flex items-center"><Speaker className="h-4 w-4 mr-2 text-indigo-400" />Backup Element</p>
                          <p className="text-white">{presenter.backup_element}</p>
                        </div>
                      )}
                      {presenter.sound_check_time && (
                        <div className="bg-gray-700/50 p-3 rounded-md">
                          <p className="text-gray-400 font-medium flex items-center"><Clock className="h-4 w-4 mr-2 text-indigo-400" />Sound Check Time</p>
                          <p className="text-white">{presenter.sound_check_time}</p>
                        </div>
                      )}
                       {presenter.presentation_type && (
                        <div className="bg-gray-700/50 p-3 rounded-md">
                          <p className="text-gray-400 font-medium flex items-center"><Users className="h-4 w-4 mr-2 text-indigo-400" />Presentation Type</p>
                          <p className="text-white">{presenter.presentation_type}</p>
                        </div>
                      )}
                      {presenter.remote_participation && (
                        <div className="bg-gray-700/50 p-3 rounded-md">
                           <p className="text-gray-400 font-medium flex items-center"><LinkIcon className="h-4 w-4 mr-2 text-indigo-400" />Remote Participation</p>
                    ...
                    {presenter.notes && (
                      <div className="mt-4 bg-gray-700/50 p-3 rounded-md">
                        <p className="text-gray-400 font-medium flex items-center"><FileText className="h-4 w-4 mr-2 text-indigo-400" />Notes</p>
              ...
              <div className="text-center py-10 bg-gray-800 rounded-lg">
                ...
                <p className="text-sm text-gray-400">This corporate mic plot currently has no presenters listed.</p>
              </div>
            )}
          </div>
        );
            ...
            <div className="bg-indigo-500/10 border border-indigo-500/30 rounded-lg p-4 mb-6 flex items-start">
              ...
              <div>
                <p className="text-indigo-300 text-sm">
                  <span className="font-medium">Shared document:</span> You are viewing a shared corporate mic plot.
